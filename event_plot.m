...
switch event
    case 1 % 20070831
        ...
        tint_zoom(5,:)=[toepoch([2007 08 31 10 18 42]) toepoch([2007 08 31 10 18 43])];
...
for i=1:size(tint_zoom,1)
    ...
    if i~=1 % E power spectrum (4 panels)
        ...
        overlap=0;
        nfft=256;
        c_eval('diE?fft=caa_powerfft(diE?zoom,nfft,fs,overlap);',3:4);
        caa_spectrogram([hca1 hca2],diE3fft);
        caa_spectrogram([hca3 hca4],diE4fft); 
        ylabel(hca1,'f [Hz]');
        ylabel(hca2,'f [Hz]');
        ylabel(hca3,'f [Hz]');
        ylabel(hca4,'f [Hz]');
    ...
    set(gcf,'PaperPositionMode','auto',...
        'Position',position);
    eval(['print -dpdf ',date,'_',num2str(i),'_',file_ext,'.pdf']);
end