...
Ay = @(x,y,z) B0*0.5*(x/a).^2 - B0*0.5*(z/b).^2;
AY0 = Ay(X,Y,Z);
...
options = odeset('Events', @(t,xyz) eom_box_edge(t,xyz,xvec([1 end])),...
                 'AbsTol',1e-6);
options = odeset('Events', @(t,xyz) eom_box_edge(t,xyz,1,[-400*1e3 1500*1e3]),...
                 'AbsTol',1e-12);
%options = odeset();



m = [units.me*10, units.mp]';
q = [-units.e, units.e]';
vz0 = E0/B0*[1 1]'; % m/s
T0 = [20 100]'; % eV
...
T = T0 + 0.3*T0.*randn([nP 1]); % eV
T(T<0) = 0;
vt = sqrt(2*T.*units.eV./m); % m/s
...
x_init_all = [repmat([100 0 z0],nP,1) vy*0 vy vz];
x_init_all = [repmat([1000e3 0 z0],nP,1) vx vy vz];
...
for ip = 1:size(x_init_all,1)
  EoM = @(t,xyz) eom(t,xyz,m(ip),q(ip),Ex,Ey,Ez,Bx,By,Bz); 
  x_init = x_init_all(ip,:);
  t_lim = [tstart(ip) tstop(ip)];
  [t,x_sol] = ode45(EoM,t_lim,x_init,options); % 
  p(ip).t = t;
  p(ip).x = x_sol(:,1);
  p(ip).y = x_sol(:,2);
  p(ip).z = x_sol(:,3);
...
if 1 % Figure
  ...
  if 0 % Bx, Bz, Ay
    ...
    plot(hca, zz*1e-3, Bx(0,0,zz)*1e9, zz*1e-3, Bz(0,0,zz)*1e9, zz*1e-3, Ey(0,0,zz)*1e3, zz*1e-3, Ez(0,0,zz)*1e3)
    irf_legend(hca,{'B_x','B_z','E_y','E_z'},[0.02 0.98])
  ...
  if 1 % Bx, Ez, Ey
    ...
    xx = linspace(-max(xlim),max(xlim),200);
    %yy = linspace(-max(ylim),max(ylim),200);
    zz = linspace(-max(zlim),max(zlim),200);
    %zz = linspace(zlim(1),zlim(2),200);
    yy = ylim;
    [X,Z] = ndgrid(xx,zz);
    AY = squeeze(Ay(X,0,Z));
    %vExBx = squeeze(Ey(X,0,Z).*Bz(X,0,Z)./sqrt(Bx(X,0,Z).^2 + Bz(X,0,Z).^2));
    Babs = sqrt(Bx(X,0,Z).^2 + Bz(X,0,Z).^2);
    vExBx = 1e-3*Ey(X,0,Z).*Bz(X,0,Z)./Babs.^2;
    
    hca.ColorOrder = pic_colors('matlab');
    pcolor(hca, X*1e-3, Z*1e-3, vExBx)
    hcb = colorbar(hca);
    hcb.YLabel.String = 'v_{ExB,x} (km/s)';
    %hca.CLim = 0.3e-3*[-10 10];
    shading(hca,'flat')
    colormap(hca,pic_colors('blue_red'))
    hold(hca,'on')
    contour(hca, X*1e-3, Z*1e-3, AY, 'k')
    hold(hca,'off')
  ...
  if 1 % (x,y,z)
    ...
    for ip = iPs
      if ip == 2; hold(hca,'on'); end
      plot3(hca,p(ip).x*1e-3,p(ip).y*1e-3,p(ip).z*1e-3,'LineWidth',2)
    end
    hold(hca,'off')  
  ...
  if 1 % (vx,vy), with contours of constant energy in vExB
    hca = h(isub); isub = isub + 1;
    

    % x0_ = [10 20 50 100 200 500];
    % Babs = sqrt(Bx(x0_,0,0).^2 + Bz(x0_,0,0).^2);
    % vExBx = 1e-3*Ey(x0_,0,0).*Bz(x0_,0,0)./Babs.^2;
    % 
    % 
    % v_rad = vExBx(1);
    % for iv = 1:numel(vExBx)      
    %   xx_ = vExBx(iv) + v_rad*cosd(0:360);
    %   yy_ = 0 + v_rad*sind(0:360);
    %   plot(hca,xx_,yy_,'--')
    % 
    %   v_rad = vExBx(iv)*(1+0.1*(iv-1));
    % 
    %   if iv == 1; hold(hca,'on'); end
    % end
    % hold(hca,'off');
    % hca.XGrid = 'on';
    % hca.YGrid = 'on';
    ...
    for ip = iPs
      plot(hca,p(ip).vx*1e-3,p(ip).vy*1e-3,'LineWidth',2)
