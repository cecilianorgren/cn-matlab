...
for it = 1:nt % perp perp
  ...
  Eperp = cross(B,cross(E,B));
  ExB = cross(E,B);
  fred = pdist.reduce('2D',Eperp,ExB,'scpot',scPot,'vint',vint_red,'vg',-30000:500:30000);
  fred.plot_plane(hca);
  colormap(hca,irf_colormap('waterfall'))
  hca.XLim = vlim_red;
  hca.YLim = vlim_red;
  hca.XLabel.String = 'v_{E_\perp}';
  hca.YLabel.String = 'v_{ExB}';
  if 1 % plot vint
    hold(hca,'on')
    plot(hca,1e-3*vint(1)*cosd(0:360),1e-3*vint(1)*sind(0:360),'k')
...
for it = 1:nt
  ...
  Eperp = cross(B,cross(E,B));
  ExB = cross(E,B);
  [ax,hcb] = mms.plot_skymap(hca,pdist,'flat','vectors',{B,'B';E,'E';Eperp,'E_\perp';ExB,'ExB'},'smooth',1);
  colormap(hca,irf_colormap('waterfall'))
end
...
hlinks3 = linkprop(h(2*nrows+(1:5)),{'View'});
%c_eval('h(?).XLim = 15*[-1 1]; h(?).YLim = 15*[-1 1];',1:nrows)
%c_eval('h(?).XLim = 15*[-1 1]; h(?).YLim = 15*[-1 1];',1:nrows)
h(11).CLim = [0 1.5]*1e-26;
