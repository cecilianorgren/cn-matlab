...
if 0 % Make the data stepfunction-like, so that it shows the accumulation time
  %%
...
if 1 % jedi-jfpe fpi flux 180 4sc, 11.25, 22.50
  isub = isub + 1;
  zoomy = [zoomy isub];
...
if 0 % E perp, abs, filt, 4 sc, time shifted for visibility
  isub = isub + 1;
  zoomy = [zoomy isub];
  hca = irf_panel('E perp abs dt filt');
  set(hca,'ColorOrder',mms_colors('1234'))
...
if 1 % edi flux 180 4sc, averaged to fpi and stepped
  ...
  set(hca,'ColorOrder',mms_colors('12'))
  %irf_legend(hca,{'EDI: \theta = [168.75 180]^o'},[0.05 0.99],'fontsize',12);
  irf_legend(hca,{'\theta = [168.75, 180]^o'},[0.05 0.99],'fontsize',12,'color',[0 0 0]);
  irf_legend(hca,{'no time shift'},[0.05 0.8],'fontsize',12,'color',[0 0 0]);
  irf_legend(hca,{'averaged to FPI timeline'},[0.98 0.98],'fontsize',12,'color',[0 0 0]);
end
