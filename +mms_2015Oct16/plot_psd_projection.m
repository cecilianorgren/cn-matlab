...
for ii = 1:nPanels, % move timeseries panels
  h1(ii).Position(3) = h1(ii).Position(3)*0.5;
...
ncols = 4;
isub = 0;
energyLevels = [6 7 8 9 10 11];

for ii = 1:nrows  
  for jj = 3:ncols
    isub = isub + 1;         
    h2(isub) = subplot(nrows,ncols,jj+(ii-1)*ncols);
    %[hca,hcb] = mms.plot_skymap(gca,desDist1,'tint',tint,'energylevel',energyLevels(isub));        
  end
end
...
switch toplot
  ...
  case 4 
    ...
    if 1
      tint = irf.tint('2015-10-16T10:33:44.60Z',0.04);
      hca = h2(isub); isub = isub + 1; 
      c_eval('mms.plot_cross_section_psd(hca,desDist?,dmpaB?brst,''tint'',tint,''scPot'',P?);',ic)
      hca.Title.String = [irf_time(tint(1).utc,'utc>utc_yyyy-mm-ddTHH:MM:SS.mmm') ' + ' num2str(tint.stop-tint.start) ' s'];
      irf_pl_mark(h1(hmark),tint.epochUnix','green')
  ...
  case 6
    ...
    vlim =10*1e3; % km/s
    xyz = mva;
    %xyz = [mva(2,:); mva(3,:); mva(1,:)];
    %xyz = [mva(3 ,:); mva(1,:); mva(2,:)];
    ...
    c_eval('E0 = mean(dslE?brst.tlim(tint).data);',ic); hatE0 = double(irf_norm(E0)); hatE0(3) = 0;    
    if norm(E0)>limE, vectors = {hatE0,'E_{z=0}'}; else vectors = {}; end
    c_eval('mms.plot_projection(hca,desDist?,''tint'',tint,''xyz'',xyz,''clim'',[0 5],''vlim'',vlim,''vectors'',vectors,''vlabel'',{''v_L'',''v_M'',''v_N''});',ic)
    irf_pl_mark(h1(hmark),tint.epochUnix','green')
    ...
    vectors = {hatB0,'B'};
    if norm(E0)>limE, vectors = {hatE0,'E_{z=0}'}; else vectors = {}; end
    c_eval('mms.plot_projection(hca,desDist?,''tint'',tint,''xyz'',xyz,''clim'',[0 5],''vlim'',vlim,''vectors'',vectors);',ic)
    irf_pl_mark(h1(hmark),tint.epochUnix','green')
