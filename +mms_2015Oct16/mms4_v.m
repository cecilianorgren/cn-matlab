...
tint = irf.tint('2015-10-16T10:20:00.00Z/2015-10-16T10:40:00.00Z');
R  = mms.get_data('R_gse',tint);
R.gsmR1 = R.gseR1;
R.gsmR2 = R.gseR2;
R.gsmR3 = R.gseR3;
R.gsmR4 = R.gseR4;
V  = mms.get_data('V_gse',tint);
V.gsmV1 = V.gseV1;
V.gsmV2 = V.gseV2;
V.gsmV3 = V.gseV3;
V.gsmV4 = V.gseV4;
...
%R.gseR1 = mean(Rxyz1.tlim(tint).data,1);
%R.gseR2 = mean(Rxyz2.tlim(tint).data,1);
%R.gseR3 = mean(Rxyz3.tlim(tint).data,1);
%R.gseR4 = mean(Rxyz4.tlim(tint).data,1);
%R.gsmR1 = mean(Rxyz1.tlim(tint).data,1);
%R.gsmR2 = mean(Rxyz2.tlim(tint).data,1);
%R.gsmR3 = mean(Rxyz3.tlim(tint).data,1);
%R.gsmR4 = mean(Rxyz4.tlim(tint).data,1);
...
if strcmp(flag,'v_from_t'),
	...
	for ic='1234',
		i=ic-'0';
R.(['vsc'  ic]) = irf_resamp([V.time.epochUnix V.([coord_sys 'V' ic])],t_center','spline');
		R.(['drsc' ic]) = irf_resamp([R.time.epochUnix R.([coord_sys 'R' ic])-R.([coord_sys 'R' num2str(1)])],t(i),'spline');
...
elseif strcmp(flag,'dt_from_v'),
  ...
  %if strcmpi(coord_sys,'gsm'), v = irf_gse2gsm([t(1) v], -1); v(1)=[]; end 
	...
	for ic='1234',
		...
		R.(['dr' ic])=irf_resamp([R.time.epochUnix R.([coord_sys 'R' ic])-R.([coord_sys 'R' num2str(1)])],t,'spline');
...
%% %%%%%%%%%%%%%%%%%%% LOCAL FUNCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function answer=is_R_ok(sc)
      ...
      if numel(R.(strSc)) < 8 % less than 2 time points
				...
				tintR=[R.time(1).epochUnix R.time(end).epochUnix];
