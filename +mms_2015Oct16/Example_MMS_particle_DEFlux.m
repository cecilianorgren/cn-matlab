...
tint = irf.tint('2015-10-16T09:29:50.00Z/2015-10-16T09:32:00.00Z');
tint = irf.tint('2015-10-16T08:19:14.00Z/2015-10-16T08:21:04.00Z');
...
%%
cmPoynting = irf_colormap('poynting');
...
hold(h(5),'on');
irf_plot(h(5),Te)
lineScpot = irf_plot(h(5),irf_ssub('P?',ic)); 
lineScpot.Color = [1 1 1]; lineScpot.LineWidth = 1.5;
irf_legend(h(5),'sc Pot',[0.8 0.2],'color','w','fontsize',12)
hold(h(5),'off');
...
hold(h(6),'on');
lineScpot = irf_plot(h(6),irf_ssub('P?',ic)); 
lineScpot.Color = [0 0 0]; lineScpot.LineWidth = 1.5;
irf_legend(h(6),'sc Pot',[0.8 95],'color','k','fontsize',12)
hold(h(6),'off');
...
colormap(h(6),cmPoynting)

h(7)=irf_panel('edistparperp');
irf_spectrogram(h(7),specparperp,'log','donotfitcolorbarlabel');
hold(h(7),'on');
lineScpot = irf_plot(h(7),irf_ssub('P?',ic)); 
lineScpot.Color = [0 0 0]; lineScpot.LineWidth = 1.5;
irf_legend(h(7),'sc Pot',[0.8 0.95],'color','k','fontsize',12)
hold(h(7),'off');
...
for kk = 1:7; h(kk).Position(3) = h(kk).Position(3)*0.85; end
