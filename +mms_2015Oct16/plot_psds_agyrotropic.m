%% Automatic results made from gui above (well it's not above anymore, but whatever)
ic = 1:4;
...
c_eval('mvaVe?=irf.ts_vec_xyz(ve?brst.time,[ve?brst.dot(mva(1,:)).data_ ve?brst.dot(mva(2,:)).data_ ve?brst.dot(mva(3,:)).data_]); mvaVe?.name = ''mva ve? brst'';')
%c_eval('mvaVe?=irf.ts_vec_xyz(ve?brst.time,[ve?brst.dot(mva(1,:)).data_ ve?brst.dot(mva(2,:)).data_ ve?brst.dot(mva(3,:)).data_]);',ic)
mvaJ=irf.ts_vec_xyz(jbrst.time,[jbrst.dot(mva(1,:)).data_ jbrst.dot(mva(2,:)).data_ jbrst.dot(mva(3,:)).data_]);
%% Initialize plot
[h1,h2] = mms.initialize_comined_plot(10,5,4,5,'vertical');

%tint = irf.tint('2015-10-16T10:33:20.00Z/2015-10-16T10:34:00.00Z');
tintDist = irf.tint('2015-10-16T10:33:30.10Z',0.04)+0.2;    
timeOverview = irf.tint('2015-10-16T10:33:25.00Z/2015-10-16T10:33:32.00Z');
tint = timeOverview;
ic = 1;
...
for ic = 1:4
  ...
  hca = h2(isub); isub = isub + 1; 
  mms.plot_projection(hca,dist,'tint',tint,'xyz',[1 0 0; hatB0;cross(hatB0,mva(2,:))],'elevationlim',elevlim,'vlim',vlim,'vectors',vectors,'usebincorrection',correctBin);
  hca.Title.String = '';
  
  hca = h2(isub); isub = isub + 1; 
  mms.plot_projection(hca,dist,'tint',tint,'xyz',[1 0 0; cross(hatB0,mva(2,:)); hatB0],'elevationlim',elevlim,'vlim',vlim,'vectors',vectors,'usebincorrection',correctBin);
  hca.Title.String = '';
end
for ii = 1:numel(h2)
  colormap(h2(ii),'jet')
...
%% Check knee distributions
tint = irf.tint('2015-10-16T10:33:45.00Z',0.2);
vlim = 15*1e3;
elevlim = 90;
correctBin = 0;
