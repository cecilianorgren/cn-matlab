...
%Bx = @(z) -B0*tanh(z*pi/d);
Bx = @(x,y,z) x*0 + y*0 - abs(z)/d*0.05*B0+- B0*tanh(z/d).*(1-exp(-z.^2/(2*b^2)))-BLoffset;
By = @(x,y,z) x*0 + y*0 + 0*z/d*0.05*B0+ Bg-3*BH*sin(4/3*z/d-zoffsy/d).*exp(-(z-zoffsy).^2/(3*b^2)).*(1-exp(-(z-zoffsy).^2/(2*b^2)));
%By = @(x,y,z) x*0 + y*0 + z*0 + Bg-7*BH*tanh(z/d).*exp(-z.^2/d^2).*(1-exp(-z.^2/(2*b^2))); 
Bz = @(x,y,z) x*0 + y*0 + z*0 + Bn;
Ex = @(x,y,z) x*0 + y*0 + z*0 + EL*(-exp(-(z.^2)/d^2));
Ey = @(x,y,z) x*0 + y*0 + z*0 + Er*(exp(-(z.^2)/d^2));
Ez = @(x,y,z) x*0 + y*0 + z*0 + E0*sin((z-dzE)/dE).*exp(-(z-dzE).^2/(2*b^2)) + Enas*exp(-(z-offsEnas).^2./d.^2);
