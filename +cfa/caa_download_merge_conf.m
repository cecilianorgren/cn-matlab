...
archive = 'caa';                        % caa is default archive to download from
%% load .caa/.cfa file with status for all downloads
if doLog,
	if exist(['.',archive],'file') == 0,
		caa=cell(0);
		save('-mat',['.',archive],archive);
	end
	load('-mat',['.',archive],archive);
end
...
if downloadFromCFA % change/add defaults, hasn't ad ded these to above flag checking
    ...
    cfaAsyncUrl = 'async-product-action?'; % used for data retrieval
...
if strfind(dataset,'list'),     % list files
	...
	disp(['Be patient! Contacting ',upper(archive),'...']);
	disp(url_line_list);
...
if ~exist(upper(archive),'dir'), mkdir(upper(archive));end    
