...
%Ne=newNeav*1e6;
%%
scaling=(Ne*mu0*e/B0)*1e18; % gives RHS in nT or LHS in volt
...
if 1 % Electric field
    ...
    c_eval('E?shift=E?zoom; E?shift(:,1)=E?shift(:,1)+t0choice;',4);
    
    hca=h(isub);isub=isub+1;
    irf_plot(hca,E3zoom(:,[1 2]),'color',gr); hold(hca,'on');
...
if 1 % Potential and dB
    ...
    irf_plot(hca,LHS,'color',[0.8 0.2 0.2]); hold on;    
    irf_plot(hca,RHS,'color',[0.8 0.8 0.2]); hold on;
    irf_zoom('x',[cn_toepoch(t1) cn_toepoch(t2)]);
    set(hca,'ColorOrder',[[0.8 0.2 0.2];[0.8 0.8 0.2]]);
    irf_legend(hca,{'phi_{\delta B} and \delta B','phi'},[0.02 0.05])
...
xy=get(gcf,'Position');
set(gcf,'Position',[xy(1) xy(2) xy(3) xy(4)*1.2])
...
eval(['print -depsc2 ',t1str,'_',t2str,'_dB_phi',Cstr,'.eps']);