function cn_edir(t1,t2,E3,E4,B3_raw,B4_raw,Vi3_raw,Vi4_raw,ExB3_raw,ExB4_raw,Pos3_raw,Pos4_raw,Te,Ti,Ne,Ni,n_hat,t1str,t2str,whichplot)

%%% Taking average B between C3 and C4 for each time step
c_eval('B?=cn_toepoch(t1,t2,B?_raw);',3:4);
Bav=irf_add(1,B3,1,B4);
B=[Bav(:,1) Bav(:,2:4)/2];

Bhat=[B(:,1) B(:,2:4)/B(:,5)]; % Bhat     



%% Defining coordinate system
z=Bhat(:,2:4);
x=irf_cross(-irf_cross(n_hat,z),z); % close to BL-normal
y=irf_cross(z,x); % close to drift direction within boundary layer
...
axis([-boxside boxside -boxside boxside]*3)
axis square

%% Plot E-direction
for l=1:3:size(E3) 
    plot(Pos3c(1),Pos3c(2),'go',Pos4c(1),Pos4c(2),'bv'); hold on;
    quiver(Pos3c(1),Pos3c(2),E3(l,2),E3(l,3),10/max(E3(:,2)))
    quiver(Pos4c(1),Pos4c(2),E4(l,2),E4(l,3),10/max(E3(:,2)))
    pause(0.1)
end
if 0% Plot ExBav
ExB=M*ExBav(2:4)';
