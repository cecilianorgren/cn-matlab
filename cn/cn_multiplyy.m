function [y]=irf_multiply(c,x1,p1,x2,p2,varargin)
...
if size(varargin,2)==0
    ...
    elseif size(x2,2)>size(x1,2),
      ...
      elseif size(x1,2)==size(x2,2)-1, x1=[x2(1,1) x1(1,:)]; % use only first row of x2, taking time from x1
        ...
        irf_log('proc','irf_multiply() ERROR: could not make inteligent guess what you are meaning.');return; 
      end
    end
...
elseif size(varargin,2)==2
    y=cn_addd(1,cn_multiplyy(1,x1,p1,x2,p2),varargin{1},varargin{2});
else
    y=cn_addd(1,cn_addd(1,x1,p1,x2,p2),varargin{1},varargin{2},varargin{3:end});
end
