...
% Properties of unperturbed plasma
units = irf_units;
n = [0.045]*1e6;
T = [400]; T_K = T*units.eV/units.kB; % use parallel temperature
vd = [-0000]*1e3; % m/s

% Derived physical quantities of unperturbed plasma
...
% Observed/measured ESW properties (konrad)
...
obs_ne_max = units.eps0/units.e*obs_potential_max./(obs_lpp/2*1e3).^2*1e-6; % cc
...
F = fe_bgk(V,n,vt,vd,PHI,VPH); % distribution function, from Schamel 1986
Ffree = fe_bgk(V,n,vt,vd,PHI,VPH,'free'); % distribution function, from Schamel 1986
Ftrap = fe_bgk(V,n,vt,vd,PHI,VPH,'trap'); % distribution function, from Schamel 1986
...
if 1 % sum F (density)
  ...
  hold(hca,'on')
  h_nmean = plot(hca,x_vec([1 end]),mean(sumFdv*1e-6)*[1 1],x_vec_diff2,ne_vec*1e-6); % 1e-6 from 1/m3 > 1/cm3
