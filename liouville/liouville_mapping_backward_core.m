...
if 1 % get for one z
  zf0 = [-00]; % starting/ending point of where to get the f
...
F_ = obsPDist(tind);
% Make one common energy array
F = F_;
F.depend{1} = repmat(F_.depend{1}(1,:),F_.length,1); F.ancillary.esteptable = zeros(F_.length,1);
...
for iPSD = 1:nPSD % do many trajectory integrations for each 'final' distribution  
  ...
  % DSL velocity corresponding to each bin
...
tsFmap = F;
tsFmap.data = allf;
tsFmap.ancillary.esteptable = zeros(numel(zf0),1);
tsZstop = F;
...
try % spis
  ...
  cd /Users/Cecilia/Data/MMS/
  save(['20151112071854_liouville_' datestr(now,'yyyy-mm-dd_HH_MM_ss') '.mat'])
end
