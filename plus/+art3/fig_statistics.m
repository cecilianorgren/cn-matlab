...
% Make power fit of values
fittall = fit([xEH xES]',[yEH yES]','power1');
fitt = fit(xEH',yEH','power1');
yPow = fitt.a*ppxx.^fitt.b;
xPow = ppxx;

% Make polynomial fit of log values
P = polyfit(log(xEH),log(yEH),1);
xP = ppxx;
...
pcoeff = 8;
xEye = ppxx;
yEye = pcoeff*(xEye).^(pexp);
%pstr = ['fit: y = ' num2str(pcoeff) '*x^{' num2str(pexp) '}'];

% Make fit by eye to model
pexp = 1;
pcoeff = 3;
xMod = ppxx;
yMod = pcoeff*(xMod).^(pexp);
strMod = ['v/v_{T} = ' num2str(pcoeff,'%.1f') '\times (v/v_{te,bg})^{' num2str(pexp,'%.1f') '}'];
...
vT = vdmat-s{4}.vph; 
plR = find(s{4}.R>0.2 & s{4}.R<0.6); 
plS = find(s{4}.S<1.6); 
xTheory = s{4}.vph(plR,plS)/s{4}.vtebg;
yTheory = s{4}.vph(plR,plS)./vT(plR,plS);
xisnan = isnan(xTheory);
yisnan = isnan(yTheory);

Pthe = polyfit(log(xTheory(~xisnan)),log(yTheory(~xisnan)),1);
xPthe = ppxx;
yPthe = exp(Pthe(1)*log(xPthe)+Pthe(2));
fittPthe = fit(xPthe',yPthe','power1');
strP = ['fit: y = ' num2str(fittPthe.a) '*x^{' num2str(fittPthe.b) '}'];
...
set(gcf,'position',[560   678   363   270]);
set(gcf,'defaultAxesFontSize',14);
set(gcf,'defaultTextFontSize',14);
set(gcf,'defaultAxesFontUnits','pixels');
set(gcf,'defaultTextFontUnits','pixels');
hold(gca,'on')
%hleg(1)=loglog(xMod,yMod,'color',[0.4 0.7 0.4],'linewidth',2); 
hleg(2)=loglog(xES,yES,'rx','linewidth',1.5,'markersize',5);
