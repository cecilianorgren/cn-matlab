...
switch lower(action)
    ...
    elseif isnumeric(time) && length(time)==1, % time given in epoch
		...
		mms.mms4_pl_conf_cn('read_position');
		mms.mms4_pl_conf_cn(plot_type);
	case 'read_position'
...
%		tint = [data.t-120 data.t+120]; % interval to read position
	...
	case 'gse'
		...
		if strcmp(data.plot_type,'lmn'), % need to redraw lmn text
			mms.mms4_pl_conf_cn('lmn');
		else
			mms.mms4_pl_conf_cn('plot');
		end
	case 'gsm'
		data=get(gcf,'userdata');
	...
	case 'default'
		...
		set(gcf,'userdata',data);
		mms.mms4_pl_conf_cn(data.coord_label);
	case 'compact'
		data=get(gcf,'userdata');
		clf;menus;
		set(gcf,'userdata',data);
  ...
  case 'lmn3d'        
    data=get(gcf,'userdata');
		...
		ss=get(0,'screensize');
		sfactor=max([1 600/(ss(3)-80) 1000/(ss(4)-80)]);
		set(gcf,'Position',[10 ss(4)-80-700/sfactor 900/sfactor 500/sfactor]);
		initialize_figure;
		h=gobjects(0);
		h(1)=axes('position',[0.1  0.16 0.4 0.7]); % [x y dx dy]
    %h(2)=axes('position',[0.69 0.16 0.5 0.2]); % [x y dx dy]
    h(2)=axes('position',[0.65 0.6 0.25 0.2]);    % for legends
		h(21) = axes('Position',get(h(1),'Position'),'XAxisLocation','top','YAxisLocation','right','Color','none','XColor','k','YColor','k');
		axis(h(2),'off');hold(h(2),'on');
		data.h=h;
		data.showClusterDescription = false; % show cluster description
		data.plot_type='lmnconfig3D';
    data.coord_label = 'LMN';
		
    callbackStr='mms.mms4_pl_conf_cn(''plot'')';
		data.LMN_text_hndl=uicontrol('string',['LMN vectors in ' data.coord_label '. One of L/M/N can be zero.'],'style','text','units','normalized','Position',[0.6 0.25 .35 .05]);
		uicontrol('string','L','style','text','units','normalized','Position',[0.6 0.2 .05 .04])
		if isfield(data,'Lstr'), Lstr=data.Lstr;else Lstr='[1 0 0]';end
		data.L_hndl=uicontrol('Style','edit','Units','normalized', ...
			'Position',[0.65 0.2 .3 .05],'String',Lstr,'Callback',callbackStr);
		uicontrol('string','M','style','text','units','normalized','Position',[0.6 0.15 .05 .04])
		if isfield(data,'Lstr'), Mstr=data.Mstr;else Mstr='[0 1 0]';end
		data.M_hndl=uicontrol('Style','edit','Units','normalized', ...
			'Position',[0.65 0.15 .3 .05],'String',Mstr,'Callback',callbackStr);
		uicontrol('string','N','style','text','units','normalized','Position',[0.6 0.1 .05 .04])
		if isfield(data,'Nstr'), Nstr=data.Nstr;else Nstr='0';end
		data.N_hndl=uicontrol('Style','edit','Units','normalized', ...
			'Position',[0.65 0.1 .3 .05],'String',Nstr,'Callback',callbackStr);
      ...
      case 'lmnconfig3D'	
		...
		if data.showClusterDescription,
			if strcmpi(data.plot_type,'compact') || ...
					strcmpi(data.plot_type,'lmn')  || ...
          strcmpi(data.plot_type,'lmn3d')% decide in which axes write labels
				hca=h(4);
			else
				hca=h(8);
			end
			...
			axis(hca,'off');
			ht=irf_legend(hca,['mms.mms4_pl_conf_cn() ' datestr(now)],[0,0],'fontsize',8);
...
if nargout,
	...
	function menus
		...
		if isempty(findobj(gcf,'type','uimenu','label','&Options'))
			...
			uimenu(hcoordfigmenu,'Label','New time','Callback','mms.mms4_pl_conf_cn(''new_time'')')
			uimenu(hcoordfigmenu,'Label','&GSE','Callback','mms.mms4_pl_conf_cn(''GSE'')','Accelerator','G')
			uimenu(hcoordfigmenu,'Label','GS&M','Callback','mms.mms4_pl_conf_cn(''GSM'')','Accelerator','M')
			uimenu(hcoordfigmenu,'Label','normal','Callback','mms.mms4_pl_conf_cn(''default'')')
			uimenu(hcoordfigmenu,'Label','config3D','Callback','mms.mms4_pl_conf_cn(''config3D'')')
			uimenu(hcoordfigmenu,'Label','lmn3D','Callback','mms.mms4_pl_conf_cn(''lmn3D'')')
			uimenu(hcoordfigmenu,'Label','compact','Callback','mms.mms4_pl_conf_cn(''compact'')')
			uimenu(hcoordfigmenu,'Label','supercompact','Callback','mms.mms4_pl_conf_cn(''supercompact'')')
			uimenu(hcoordfigmenu,'Label','supercompact2','Callback','mms.mms4_pl_conf_cn(''supercompact2'')')
			uimenu(hcoordfigmenu,'Label','LMN','Callback','mms.mms4_pl_conf_cn(''lmn'')')
			uimenu(hcoordfigmenu,'Label','New sc_list','Callback','mms.mms4_pl_conf_cn(''new_sc_list'')')
