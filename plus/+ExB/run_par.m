...
turns = round(vz0box*1e3*T/(L*1e3));
turns = abs(turns);
nParticlesFlow = sum(turns);

% Add that number of particles with that velocity to a new vector 
vz0flow = [];
for kk = 1:nParticlesBox
    vz0flow = [vz0flow; repmat(vz0box(kk),turns(kk),1)];    
end
...
npar = 12; % number of parallel runs
tend = zeros(nParticles,1);
% Divide particles into 12 groups and run on parallel cores
nGroup = floor(nParticles/12);
nParallelParticles = ones(npar,1)*nGroup;
nTot = sum(nParallelParticles);
nParallelParticles(npar) = nParallelParticles(npar) + mod(nParticles,npar);
parfor ip = 1:12        
  %  tendTemp = zeros(nParallelParticles(ip),1);
  %  nBounceTemp = zeros(nParallelParticles(ip),1);
    sumMVrzTemp = zeros(nr,nz);
    sumMrzTemp = zeros(nr,nz);
    sumMVxyzTemp = zeros(nx,ny,nz);
    sumMxyzTemp = zeros(nx,ny,nz);
    tint = [0 T];
    for jj=1:nParticles     
 ...
 %   try               
        ...
        % defined in options.
        [t,x_sol] = ode45(@ExB.EquationOfMotion,tint,x_init,options);
        x = x_sol(:,1);
        y = x_sol(:,2);
        z = x_sol(:,3);
        vx = x_sol(:,4);
       ...
       % nBounceTemp(jj) = numel(find(diff(sign(vz))~=0)); 
        ...
        if rzbinning % rz binning
            ...
            for ui = 1:nun                  
                ...
                sumMVrzTemp(ir,iz) = sumMVrzTemp(ir,iz) + mean(vaztmp(ic)).*mass;
                sumMrzTemp(ir,iz) = sumMrzTemp(ir,iz) + mass;
        ...
        if xyzbinning % xyz binning            
            ...
            for ui = 1:nun      
                ...
                sumMVxyzTemp(ix,iy,iz) = sumMVxyzTemp(ix,iy,iz) + mean(vaztmp(ic)).*mass;
                sumMxyzTemp(ix,iy,iz) = sumMxyzTemp(ix,iy,iz) + mass;                
    ...
    sumMVrz  = sumMVrz  + sumMVrzTemp;
    sumMrz   = sumMrz   + sumMrzTemp;
    sumMVxyz = sumMVxyz + sumMVxyzTemp;
    sumMxyz  = sumMxyz  + sumMxyzTemp;
  %  nBounce((ip-1)*nParallelParticles(1)+1:nParallelParticles(ip))= nBounceTemp;
  %  tend((ip-1)*nParallelParticles(1)+1:nParallelParticles(ip))= tendTemp;
end % end parfor
