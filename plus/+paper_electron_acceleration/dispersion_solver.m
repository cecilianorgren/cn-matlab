...
%% Argument to disperion equation, to see what terms dominate
Nfad = 50;
...
nsp = numel(vt);
xis_real = zeros(nsp,nk);
xis_imag = zeros(nsp,nk);
for isp = 1:nsp
  xis_real(isp,:) = xi(wr_store,kvec,vt(isp),wp(isp),vd(isp));
  xis_imag(isp,:) = xi(1i*wi_store,kvec,vt(isp),wp(isp),vd(isp));
end
...
if 0 % xis
  ...
  plot(hca,kvec*1e3,imag(xis_real),'-',kvec*1e3,imag(xis_imag),'--')
  %plot(hca,kvec*1e3,real(xis_real),'-',kvec*1e3,real(xis_imag),'--')
  hleg = legend(hca,{'real1','real2','real3','imag1','imag2','imag3'});
  hleg.Title.String = 'species';
  hca.XLabel.String = 'k (mk^{-1})';
  hca.XLabel.String = 'xi';
end
