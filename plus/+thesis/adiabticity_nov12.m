...
npanels = 6;
tintZoom = irf.tint('2015-11-12T07:19:20.00Z/2015-11-12T07:19:24.10Z');
...
if 1 % length scales
  ...
  if 1 % estimate magnetic length scale
    ...
    c_eval('irf_plot(hca,{re?,LB?},''comp'');',ic)
    irf_legend(hca,{'r_e','L_B'},[0.95 0.9],'fontsize',12);
  else
...
if 1 % e DEF omni 64
  hca = irf_panel('e DEF omni 64');   isub = isub + 1;  zoomE = [zoomE isub];
  ...
  colormap(hca,cmap) 
  irf_legend(hca,{'0<\theta<180'},[0.98 0.15],'fontsize',12,'color',[0 0 0]);
end
...
if 1 % eDist omni 64 perp
  ...
  if 1 % plot perpendicular energy based on magnetic moment conservation
    ...
    Efactor = 3;
    c_eval('lineWperp = irf_plot(hca,Wperp*Efactor,''k'');',ic)  
    %lineScpot.Color = [0 0 0]; lineScpot.LineWidth = 1.5;
    irf_legend(hca,{sprintf('%.0f E_{e,ref}',Efactor)},[0.77 0.2])
    irf_plot(hca,Wperp.resample(tref)*Efactor,'ok')
...
if 0 % eDist omni 64 perp 30 60
  hca = irf_panel('e DEF perp3'); isub = isub + 1;  zoomE = [zoomE isub];    
  pas = [30 60];
  c_eval('ePitch?lim = ePDist?.pitchangles(dmpaB?,pas);',ic)
...
if 0 % eDist omni 64 perp perp 60-90
  hca = irf_panel('e DEF perp2'); isub = isub + 1;  zoomE = [zoomE isub];    
  pas = [60 90];
  c_eval('ePitch?lim = ePDist?.pitchangles(dmpaB?,pas);',ic)
...
if 1 % ePDist pa low energies
  ...
  hca.XGrid = 'off';
  hca.YGrid = 'off';
  irf_legend(hca,{'\theta'},[0.38 0.4],'fontsize',24) 
...
irf_zoom(h([1:3 10:end]),'y')
for  ii = zoomE; h(ii).YLim = [10 1000]; end

for  ii = zoomE(1:2); h(ii).CLim = [6 8.2]; end
...
hl = add_length_on_top(h(1),70,0.2);
hl.XLabel.String = 'km';
...
if 1 % eDist omni 64 par
  ...
  if 1 % plot perpendicular energy based on magnetic moment conservation   
    ...
    hold(hca,'on')        
    c_eval('lineWperp = irf_plot(hca,Wpar*Efactorpar,''k'');',ic)  
    irf_plot(hca,irf.ts_scalar(tref,Wrefpar),'*k')
    hleg = irf_legend(hca,{'E_{\mu}'},[0.70 0.2],'color',[0 0 0],'fontsize',12);
    hold(hca,'off')    
    hca.XGrid = 'off';
    hca.YGrid = 'off';
  end
...
if 1 % ePDist pa low energies
  %%
  hca = irf_panel('e PA e32 deflux all low E low');  isub = isub + 1;  zoomPA = [zoomPA isub];
  % find magnetic mirror angle
  tref4 = irf_time('2015-11-12T07:19:21.40Z','utc>epochtt'); tlength4 = [-0.5 0];
  tref3 = irf_time('2015-11-12T07:19:21.40Z','utc>epochtt'); tlength3 = [-0.5 0];
  tref2 = irf_time('2015-11-12T07:19:21.50Z','utc>epochtt'); tlength2 = [-0.60 0];
...
irf_plot_axis_align
%for ii = 1:3; h(ii).Visible = 'off'; eand
...
%print('-dpng','-painters',[thesis.path 'fig_e_mu_mms1_.png'])