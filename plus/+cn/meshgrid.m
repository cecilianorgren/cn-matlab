...
if nargin == 2 || (nargin == 1 && nargout < 3) % 2-D array case
    ...
    if isempty(x) || isempty(y)
        ...
        xcol = full(x(:)); % Make sure x is a full column vector.
        yrow = full(y(:)).';   % Make sure y is a full row vector.
        xx = repmat(xcol,size(yrow));
        yy = repmat(yrow,size(xcol));
    end
else  % 3-D array case
    if nargin == 1
        y = x;
    ...
    if isempty(x) || isempty(y) || isempty(z)
        ...
        ny = numel(y);
        nz = numel(z);
        xx = reshape(full(x),[nx 1 1]); % Make sure x is a full row vector.
        yy = reshape(full(y),[1 ny 1]); % Make sure y is a full column vector.
        zz = reshape(full(z),[1 1 nz]); % Make sure z is a full page vector.
        xx = repmat(xx, 1, ny, nz);
        yy = repmat(yy, nx, 1, nz);
        zz = repmat(zz, nx, ny, 1);
    end
end
