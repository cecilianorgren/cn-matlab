function [download_status,downloadfile]=download(tint,dataset,varargin)
% CFA.DOWNLOAD Download CAA data in CDF format
%       CFA.DOWNLOAD - check the status of jobs in current directory
%
%       CFA.DOWNLOAD('list')    - list all datasets and their available times
%       TT=CFA.DOWNLOAD('list')- return time table with datasets and available times
%       CFA.DOWNLOAD('listdesc')- same with dataset description
%       CFA.DOWNLOAD('listgui') - same presenting output in separate window
%       CFA.DOWNLOAD('list:dataset')- list:/listdesc:/listgui:  filter datasets 'dataset'
%       TT=CFA.DOWNLOAD('listdata:dataset') - return timetable with intervals when dataset has data
%
%       CFA.DOWNLOAD(tint,dataset) - download datasets matching 'dataset'
%       CFA.DOWNLOAD(tint,dataset,flags) - see different flags below
%
%       TT=CFA.DOWNLOAD(tint,'list') - inventory of all datasets available, can return time table TT
%       TT=CFA.DOWNLOAD(tint,'list:dataset') - only inventory datasets matching 'dataset'
%
%       download_status=CFA.DOWNLOAD(tint,dataset) - returns 1 if sucessfull download
%				returns 0 if request is put in the queue,
%				the information of queued requests is saved in file ".caa"
%		[download_status,downloadfile]=CFA.DOWNLOAD(tint,dataset) - returns also
%				zip file link if request put in queue (good for batch processing)
%       download_status=CFA.DOWNLOAD(tint,dataset,input_flags) see list of Input flags
%
%       CFA.DOWNLOAD(url_string) - download CAA data zip file from the link "url_string"
%
...
%						check the readiness by executing CFA.DOWNLOAD from the same direcotry
...
%
%  Examples:
%   CFA.DOWNLOAD(tint,'list:*')       % list everything available from all sc
%   CFA.DOWNLOAD(tint,'list:*FGM*')
%   CFA.DOWNLOAD('2005-01-01T05:00:00.000Z/2005-01-01T05:10:00.000Z','list:*FGM*')
%   CFA.DOWNLOAD(tint,'C3_CP_FGM_5VPS')
%   CFA.DOWNLOAD(tint,'C?_CP_FGM_5VPS')   %    download all satellites
%
% The example list of datasets: (see also http://bit.ly/pKWVKh)
% FGM
%   CFA.DOWNLOAD(tint,'C?_CP_FGM_5VPS');
%   CFA.DOWNLOAD(tint,'C?_CP_FGM_FULL');
% EFW (L2 - full resolution, L3 - spin resolution)
%   CFA.DOWNLOAD(tint,'C?_CP_EFW_L?_E3D_INERT'); % Ex,Ey,Ez in ISR2
%   CFA.DOWNLOAD(tint,'C?_CP_EFW_L?_E3D_GSE'); % Ex,Ey,Ez in GSE
%   CFA.DOWNLOAD(tint,'C?_CP_EFW_L?_E'); % Ex,Ey in ISR2
%   CFA.DOWNLOAD(tint,'C?_CP_EFW_L?_P'); % satellite potential
%   CFA.DOWNLOAD(tint,'C?_CP_EFW_L?_V3D_GSE'); % ExB velocity GSE
%   CFA.DOWNLOAD(tint,'C?_CP_EFW_L?_V3D_INERT'); % ExB velocity ISR2
% STAFF
%   CFA.DOWNLOAD(tint,'C?_CP_STA_PSD');
%   CFA.DOWNLOAD(tint,'*STA_SM*');           % STAFF spectral matrix
% WHISPER
%   CFA.DOWNLOAD(tint,'C?_CP_WHI_NATURAL');
% CIS
%   CFA.DOWNLOAD(tint,'C?_CP_CIS_HIA_ONBOARD_MOMENTS');
%   CFA.DOWNLOAD(tint,'C?_CP_CIS_CODIF_HS_H1_MOMENTS');
%   CFA.DOWNLOAD(tint,'C?_CP_CIS_HIA_HS_1D_PEF');
%   CFA.DOWNLOAD(tint,'C?_CP_CIS_CODIF_H1_1D_PEF');
% PEACE
%   CFA.DOWNLOAD(tint,'C?_CP_PEA_PITCH_SPIN_DPFlux'); % DPFlux/DEFLux/PSD
%   CFA.DOWNLOAD(tint,'C?_CP_PEA_3DR?_PSD');
%   CFA.DOWNLOAD(tint,'C?_CP_PEA_MOMENTS')
% RAPID
%   CFA.DOWNLOAD(tint,'C?_CP_RAP_ESPCT6'); % electron omni-directional
%   CFA.DOWNLOAD(tint,'C?_CP_RAP_L3DD');   % electron, 3D distribution (standard)
%   CFA.DOWNLOAD(tint,'C?_CP_RAP_E3DD');   % electron, 3D distr. (best) in BM
%   CFA.DOWNLOAD(tint,'C?_CP_RAP_HSPCT');  % ion, omni-directional
% EPHEMERIS
%   CFA.DOWNLOAD(tint,'C?_CP_AUX_POSGSE_1M');  % position & velocity for each sc
%   CFA.DOWNLOAD(tint,'CL_SP_AUX');            % position,attitude.. for all sc
%   CFA.DOWNLOAD(tint,'C?_CP_AUX_SPIN_TIME');  % spin period, sun pulse time,..
%   CFA.DOWNLOAD(tint,'C?_JP_PMP');            % invariant latitude, MLT, L shell.
...
retrievalType           = '&PRODUCT';   % default is to download, not check inventory, hmmmmmmmmmm
urlFileInterval='&file_interval=72hours'; % default time interval of returned files
urlFileInterval='';                     % do not use
urlSchedule='';							% default do not have schedule option
urlFormat='&DELIVERY_FORMAT=CDF';				% default is CDF (3.3) format
caaServer='http://cfadev.esac.esa.int/';	% default server
cfaUrl='cfa/aio/product-action?';       % used for both inventory check and data retrieval
...
if nargin>=1, % check if fist argument is not caa zip file link
	if ischar(tint) && any(regexp(tint,'\.zip'))% tint zip file link
		...
		if doLog
			...
			zipFileLink=tint;
			isJobFinished=get_gz_file(zipFileLink);
			if ~isJobFinished, %
...
%% Check status of downloads if needed  ==> DO NOT IMPLEMENT AT THE MOMENT 
if doLog && checkDownloadStatus,    % check/show status of downloads from .caa file
	...
	for j=1:length(caa), % go through jobs
		...
		elseif strcmpi(caa{j}.status,'submitted'),
			...
			isJobFinished=get_gz_file(caa{j}.zip);
			if isJobFinished, %
...
if isnumeric(tint) && (size(tint,2)==2), % assume tint is 2 column epoch
    ...
    nChar=numel(tintiso);
    t1iso = tintiso(1:floor(nChar/2));
    t2iso = tintiso(ceil(nChar/2+1):nChar);
...
elseif isempty(tint) % will only list products
	...
	disp(tint);
	error('cfa.download: unknown tint format');
end
...
%% List data if required ==> DO NOT IMPLEMENT AT THE MOMENT 
if strfind(dataset,'list'),     % list files
	...
	if strfind(dataset,'listdata')
		ttTemp = cfa.download(['list:' filter]);
		tint = ttTemp.TimeInterval(1,:);
		ttTemp = cfa.download(tint,['list:' filter]);
...
if 0; % checkIfDataAreAtCaa % skip at the moment
    
	url_line_list=[ caaServer cfaUrl urlIdentity '&RETRIEVALTYPE=INVENTORY' '&dataset_id=' dataset '&START_DATE=' t1iso '&END_DATE=' t2iso];
	disp(url_line_list);
...
url_line=[caaServer cfaUrl urlIdentity '&dataset_id=' ...
	dataset '&START_DATE=' t1iso '&END_DATE=' t2iso urlFormat urlFileInterval urlNonotify urlSchedule];

disp('Be patient! Submitting data request to CAA...');
disp(url_line);

[status,downloadedFile] = get_gz_file(url_line);
...
if status == 0 && exist(downloadedFile,'file')
	...
	if exist('downloadfile','var'),
		if doLog
			...
			disp('To check the status of jobs execute: cfa.download');
...
%% Functions
	function [status,gzFileName]=get_gz_file(urlLink)
		% download gz file, if succeed status=1 and file is unzipped and moved
...
%caaUser=[];
if isempty(caaUser)
	caaUser = input('Input caa username [default:avaivads]:','s');
	if isempty(caaUser),
...
%caaPwd = [];
if isempty(caaPwd)
	caaPwd = input('Input caa password [default:!kjUY88lm]:','s');
	if isempty(caaPwd), caaPwd='!kjUY88lm';end
