...
% ----------------------------------------------------------------------------
  ...
  methods
    ...
    function Ts = mtimes(obj1,obj2)
      ...
      % Can not use this anymore since it messes up matrix multiplication
      %if isa(obj1,'TSeries') && ~isa(obj,'TSeries')
      %  obj1Tmp = obj1; obj1 = obj; obj = obj1Tmp;
      %end
      ...
      if any([isnumeric(obj1) isnumeric(obj2)]) || (isa(obj1,'TSeries') && obj1.tensorOrder == 0) || (isa(obj2,'TSeries') && obj2.tensorOrder == 0)
        1
      end
      if isa(obj1,'TSeries')
      ...
      if isa(obj1,'TSeries')
        sizeObj1 = size(obj1.data);         
        nTimes = sizeObj1(1);
        sizeObj1(1) = 1; % put time as dim = 1
        data1 = permute(obj1.data,[2:length(sizeObj1) 1]); % move time to last index
        data1 = reshape(data1,[sizeObj1 nTimes]);
      ...
      if isa(obj2,'TSeries')
        sizeObj2 = size(obj2.data); 
        nTimes = sizeObj1(1);
        sizeObj2(1) = 1; % put time as dim = 1
        data2 = permute(obj2.data,[2:length(sizeObj2) 1]); % move time to last index
        data2 = reshape(data2,[sizeObj2 nTimes]);
      ...
      if to1 == 0 && to2 == 0 % obj1 and obj2 scalar
        if isa(obj1,'TSeries') && isa(obj2,'TSeries') && obj1.time~=obj2.time
          if sizeObj1(2) ~= sizeObj2(1);
            error('Size of scalar fields must agree.')
          end
          newData = data1.*data2;
        end
      elseif to1 == 0 && to2 > 0 % obj1 scalar
        if sizeObj1 ~= sizeObj2
          error('Multiplication of scalar with tensor (order > 0) requires a single scalar.')
        end      
        newData = data2*data1;
      elseif to1 > 0 && to2 == 0 % obj2 scalar
        if sizeObj2 ~= sizeObj1
          error('Multiplication of scalar with tensor (order > 0) requires a single scalar.')
        end      
        newData = data1*data2;
      else % obj1 and obj2 are both non-scalars
        % See if the dimensions are compatible  
        if sizeObj1(2) ~= sizeObj2(1);
        ...
        % Perform 'tensor notation' multiplication for tensor orders>0
        newData = zeros(sizeObj1(1),sizeObj2(2),nTimes);
        for i1 = 1:sizeObj1(1);
          for i2 = 1:sizeObj2(2);
            for indSum = 1:sizeObj1(2);
              newData(i1,i2,:) = newData(i1,i2,:) + data1(i1,indSum,:).*data2(indSum,i2,:);            
            end
          end
        end

        newData = squeeze(permute(newData,[length(size(newData)) 1:length(size(newData))-1]));
      ...
      function determine_tensororder
      end     
