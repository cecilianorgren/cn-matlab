...
if 1 % Spectra figure for paper with integration in Fourier space
    ...
    doSmooth = 1;
    nantreatment = 'interp';
    v_factor = 430/velocity;1;430/320;320/430; % multiply v with this to adjust for other time intervals than tint
    ...
    figure(44);hh=irf_plot({locBz,BDC})
    ...
    EFWfilter = 180;
    kfilter = EFWfilter*2*pi/velocity*di_loc;
    %lengthcolors = [1 0 0; 0 1 0; 0 0 1];
...
if 1 % Spectra figure for paper with integration in Fourier space and units mV/m
    %% Some input
    figure(53)
    flim = 0.1; % for correlation and velocity
...
%     0.6350    0.0780    0.1840]
    ...
    phiBscale = B0*1e-9/(n_loc*1e6*e*4*pi*1e-7)/(velocity*v_factor);
    phiEscale = 1;
    phiEscale = 1; % omega/v = 2*pi*f/v
    ...
    pfftBz = irf_powerfft(locBz,size(locBz,1),450,0.5);
    pfftPhiBbetter = irf_powerfft(locPhiB,size(locPhiB,1),450,0.5);
    pfftPhiBbetter.p{1} = pfftPhiBbetter.p{1};%/((velocity*v_factor/2/pi./pfftPhiBbetter.f').^2);
    ...
    kscale = 1;di_loc;
    hca = subplot(1,1,1);
    lines = loglog(hca,plotPhiBf*2*pi/velocity*kscale,plotPhiBp,'-',...
                       plotPhiEf*2*pi/velocity*kscale,plotPhiEp,'-');
    ...
    hca.YLabel.String = 'Power [V^2 /Hz]';
   %hca.YLabel.String = 'Power Spectral Density [V^2 /Hz]';
    if (kscale-1 == 0); hca.XLabel.String = 'k_{\perp}'; else hca.XLabel.String = 'k_{\perp}d_i'; end
    hca.Title.String = tint_str;
    %hca.XLim = [3e-2 1e3]/kscale;
    hca.YLim = [1e-6 1e10];
    tickstep = 1;
    %hca.YTick = 10.^(log10(hca.YLim(1)):tickstep:log10(hca.YLim(2)));
    hca.YTick = 10.^(-16:2:16);
    ...
    hold(hca,'on')
    hca.XLim = [3e-2 1e3]*(kscale)/di_loc;   
    
    lines = loglog(kscale/ri_loc*[1 1],hca.YLim,... 
                   kscale/sqrt(ri_loc*re_loc)*[1 1],hca.YLim,...
                   kscale/re_loc*[1 1],hca.YLim);
    ...
    %hca.XLim = klims;       
    %h_leg = legend(hca,{'B_{||}\times(B_0/n_ee\mu_0)','\int E_k dt\times v'},'location','northeast','box','off','fontsize',fontsize);
    h_leg = legend(hca,{phi_labels{1},'{E_{\perp}}'},'location','northeast','box','off','fontsize',fontsize);
    ...
    halign = 'left';
    textposition = 3*10^-4;hca.YLim(1)+diff(log10(hca.YLim))*0.1; 
    th = text(kscale/ri_loc,textposition,' k_{\perp}r_i=1','color',lengthcolors(2,:),'fontsize',fontsize,'horizontalalignment',halign);
    th = text(kscale/sqrt(ri_loc*re_loc),textposition,' k_{\perp}(r_er_i)^{1/2}=1','color',lengthcolors(3,:),'fontsize',fontsize,'horizontalalignment',halign);
    th = text(kscale/re_loc,textposition,' k_{\perp}r_e=1','color',lengthcolors(4,:),'fontsize',fontsize,'horizontalalignment',halign);
    ...
    %% Make fit to spectra
    % Magnetic field
    fitx = tocolumn(plotPhiBf*2*pi/velocity*kscale);
    fity = tocolumn(plotPhiBp);
    plotoff=-1;
      
    low_ind = find(fitx<kscale/re_loc);
    high_ind = find(fitx>kscale/re_loc);
    
    F0 = fit(log10(fitx(low_ind)),log10(fity(low_ind)),'poly1')
    hold(hca,'on')
    loglog(hca,fitx(low_ind),10.^(F0.p1*log10(fitx(low_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(low_ind(fix(end/2))),10.^(F0.p1*log10(fitx(low_ind(fix(end/2))))+F0.p2-plotoff),['k^{' num2str(F0.p1) '}'],'color','k','fontsize',fontsize,'verticalalignment','bottom');
    
    F0 = fit(log10(fitx(high_ind)),log10(fity(high_ind)),'poly1')
    hold(hca,'on')
    loglog(hca,fitx(high_ind),10.^(F0.p1*log10(fitx(high_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(high_ind(fix(end/2))),10.^(F0.p1*log10(fitx(high_ind(fix(end/2))))+F0.p2-plotoff),['k^{' num2str(F0.p1) '}'],'color','k','fontsize',fontsize,'verticalalignment','bottom');
    hold(hca,'off')
    
    % Electric field
    fitx = tocolumn(plotPhiEf*2*pi/velocity*kscale);
    fity = tocolumn(plotPhiEp);  
    plotoff=1;  
    ...
    hold(hca,'on')
    loglog(hca,fitx(low_ind),10.^(F0.p1*log10(fitx(low_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(low_ind(fix(end/2))),10.^(F0.p1*log10(fitx(low_ind(fix(end/2))))+F0.p2-plotoff),['k^{' num2str(F0.p1) '}'],'color','k','fontsize',fontsize,'verticalalignment','top');       
    
    F0 = fit(log10(fitx(high_ind)),log10(fity(high_ind)),'poly1')
    hold(hca,'on')
    loglog(hca,fitx(high_ind),10.^(F0.p1*log10(fitx(high_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(high_ind(fix(end/2))),10.^(F0.p1*log10(fitx(high_ind(fix(end/2))))+F0.p2-plotoff),['k^{' num2str(F0.p1) '}'],'color','k','fontsize',fontsize,'verticalalignment','top');
    hold(hca,'off')
    
    hold(hca,'on')
    %loglog(hca,fitx(high_ind),10.^(F0.p2-plotoff)+(fitx(high_ind)).^(F0.p1),'--','color','k')
    hold(hca,'off')
    
    %% Make electric field and magnetic field spectra with fits
    fig = figure(45);
    fig.Position = [589 450 538 248];
    fontsize = 14;
    klims = [10^-1 7*10^2];    
    ...
    plotEf(idxNan) = [];
    plotEp(idxNan) = [];
    idxBNan = isnan(plotBp);
    % B
    plotBf(idxBNan) = [];
    plotBp(idxBNan) = [];    
    % remove values above 180 Hz filter
    % E
    idxE180 = find(plotEf>180);
    plotEf(idxE180) = [];
    plotEp(idxE180) = [];
    % B
    idxB180 = find(plotBf>180); 
    plotBf(idxB180) = [];   
    plotBp(idxB180) = [];
    % remove frequencies which is above the magnetic field noise floor, ~80
    % Hz, see figure that can be plottet a few lines below
    idxBnoise = find(plotBf>80);
    plotBp(idxBnoise) = [];
    plotBf(idxBnoise) = [];
    
    % remove frequencies which are below 0.03 Hz 
    lowestf = 0.03;
    idxBlow = find(plotBf<lowestf);
    plotBp(idxBlow) = [];
    plotBf(idxBlow) = [];
    idxElow = find(plotEf<lowestf);
    plotEp(idxElow) = [];
    plotEf(idxElow) = [];
    ...
    kscale = di_loc;
    hca = subplot(1,1,1);
    [ax,hB,hE] = plotyy(plotBf*2*pi/velocity*kscale,plotBp,...
                        plotEf*2*pi/velocity*kscale,plotEp,'loglog','loglog');               
    ax(1).YColor = [0 0 0];             
    ax(2).YColor = [0 0 0];                                 
    hB.Color = colors_phi(2,:);
    hE.Color = colors_phi(1,:);
    
    ax(1).YLabel.String = 'B [(nT)^2/Hz]'; ax(1).YLabel.FontSize = fontsize;
    ax(2).YLabel.String = 'E [(mV/m)^2/Hz]'; ax(2).YLabel.FontSize = fontsize;
    
    ax(1).Title.String = ['Power spectral density: ' tint_str];  ax(1).Title.FontSize = fontsize;
   ...
   %hca.YLabel.String = 'Power Spectral Density [V^2 /Hz]';
    ...
    axB = ax(1);
    axB.YLim = [1e-7 1e12];
    axB.YTick = 10.^(-16:2:16);
    axB.YLabel.FontSize = fontsize;
    axB.Box ='off';
    
    axE = ax(2);
    axE.YLim = [1e-6 1e3];
    axE.YTick = 10.^(-16:2:16);
    axE.YLabel.FontSize = fontsize;
    %axE.Box ='on';
    
    
    hold(axB,'on'); hold(axE,'on')
    axB.XLim = [3e-2 1e3]*(kscale)/di_loc;   
    axE.XLim = [3e-2 1e3]*(kscale)/di_loc;   
    
    linesk = loglog(kscale/ri_loc*[1 1],hca.YLim,... 
                   kscale/sqrt(ri_loc*re_loc)*[1 1],hca.YLim,...
                   kscale/re_loc*[1 1],hca.YLim);

    linesk(1).Color = lengthcolors(2,:);
    linesk(2).Color = lengthcolors(3,:);
    linesk(3).Color = lengthcolors(4,:);
    %linesk(4).Color = [0 0 0];
    %lines = loglog(kfilter*[1 1],hca.YLim,'k--');
    hold(hca,'off')
    ...
    %hca.XLim = klims;       
    %h_leg = legend(hca,{'B_{||}\times(B_0/n_ee\mu_0)','\int E_k dt\times v'},'location','northeast','box','off','fontsize',fontsize);
    h_leg = legend([hB hE],{'B_{||}','E_{\perp}'},'location','northeast','box','off','fontsize',fontsize);
   
   
    
    % Add length scale legends directly to figure
    halign = 'right';
    textposition = 2*10^-5;hca.YLim(1)+diff(log10(hca.YLim))*0.1; 
    th = text(kscale/ri_loc,textposition,' k_{\perp}r_i=1 ','color',lengthcolors(2,:),'fontsize',fontsize,'horizontalalignment',halign);
    th = text(kscale/sqrt(ri_loc*re_loc),textposition,' k_{\perp}(r_er_i)^{1/2}=1 ','color',lengthcolors(3,:),'fontsize',fontsize,'horizontalalignment',halign);
    th = text(kscale/re_loc,textposition,' k_{\perp}r_e=1 ','color',lengthcolors(4,:),'fontsize',fontsize,'horizontalalignment',halign);
    ...
    if 0
    topax = axes('Position',get(ax(1),'Position'));
    topax.Position = axE.Position;
    topax.YLim = axE.YLim;
    topax.YTick = axE.YTick;
    end
    
    
    % Make fit to spectra
    % Magnetic field
    fitx = tocolumn(plotBf*2*pi/velocity*kscale);
    fity = tocolumn(plotBp);
    plotoff=1;
    hca = axB;
    ...
    hold(hca,'on')
    loglog(hca,fitx(low_ind),10.^(F0.p1*log10(fitx(low_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(low_ind(fix(0.5*end/2))),10.^(F0.p1*log10(fitx(low_ind(fix(end/2))))+F0.p2-plotoff),['k^{' num2str(F0.p1,'%.2f') '}'],'color','k','fontsize',fontsize,'verticalalignment','top');
    
    F0 = fit(log10(fitx(high_ind)),log10(fity(high_ind)),'poly1')
    hold(hca,'on')
    loglog(axB,fitx(high_ind),10.^(F0.p1*log10(fitx(high_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(high_ind(fix(1*end/2))),10.^(F0.p1*log10(fitx(high_ind(fix(end/2))))+F0.p2-plotoff-2),['k^{' num2str(F0.p1,'%.2f') '}'],'color','k','fontsize',fontsize,'verticalalignment','top');
    hold(hca,'off')
    %
    % Electric field
    fitx = tocolumn(plotEf*2*pi/velocity*kscale);
    fity = tocolumn(plotEp);  
    plotoff=-1;  
    hca = axE;
    low_ind = find(fitx<kscale/re_loc);
    high_ind = find(fitx>kscale/re_loc);
    ...
    axis(hca);
    loglog(hca,fitx(low_ind),10.^(F0.p1*log10(fitx(low_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(low_ind(fix(1.4*end/2))),10.^(F0.p1*log10(fitx(low_ind(fix(end/2))))+F0.p2-plotoff*8),['k^{' num2str(F0.p1,'%.2f') '}'],'color','k','fontsize',fontsize,'verticalalignment','bottom');       
    
    F0 = fit(log10(fitx(high_ind)),log10(fity(high_ind)),'poly1')
    hold(hca,'on')
    loglog(hca,fitx(high_ind),10.^(F0.p1*log10(fitx(high_ind))+F0.p2-plotoff),'--','color','k')
    th = text(fitx(high_ind(fix(end/2))),10.^(F0.p1*log10(fitx(high_ind(fix(end/2))))+F0.p2-plotoff*6),['k^{' num2str(F0.p1,'%.2f' ) '}'],'color','k','fontsize',fontsize,'verticalalignment','bottom');
    hold(hca,'off')
