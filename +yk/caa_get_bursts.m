...
if flag_local
    old_pwd = '.';
    [loc_st,loc_dt] = caa_read_interval; % F?rst?r inte vad denna g?r
    if isempty(loc_st)
        irf_log('proc','No Cluster data in the current directory (.interval not found)');
        ret=2;
        return;
        % CN: S? om det inte finns n?gon Clusterdata i current directory s?
        % funkar inte denna funktion, dvs den k?r return.. S? jag m?ste
        % ladda in data fr?n n?got annat s?tt, dvs. CAA.
    end
...
% CN: Det var det h?r Yuri pratade om.
if varsbsize>=2
    can_check_order = 1;
    ...
    if can_check_order
        ...
        % CN: Det h?r borde vara samma som informationen man kan hitta p?
        % efw/ops-sidan v?l?
        probes = [];
    ...
    if can_check_order
        ...
        % CN: G?r an lista med alla probpar vi vill/kan anv?nda
        probep = [];
    ...
    if can_check_order
        % Load NM data
        % CN: NM-data = Normal mode data? F?r att kunna j?mf?ra?
        ...
        if isempty(nmdata)
            ...
            % CN: ok, detta ?r f?r att j?mf?ra och se om V1 verkligen st?r p?
            % V1-platsen
        end
    end
...
% CN: This is what you get stright away from CAA, or atleast some proxy of
% it, assuming the data at CAA is ordered, which it isn't necessarily
...
for i=1:varsbsize
    if varsb{i}(1)=='B' % BP data
        % CN: BP data wasnt supposed to be of any importance to me
        % according to Yuri.
    ...
    elseif varsb{i}(1)=='V'
        if length(varsb{i})>3 % Differential signals are special
            ...
            if probe==12 || probe==43
                ...
                % 0.00212 ?ndrar fr?n telemetrienheter till fysikaliska
                % enheter
            else
...
for i=1:varsbsize
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%% PROBE MAGIC %%%%%%%%%%%%%%%%%%%%%%
    % CN: Since one probe is broken on one of the sc, the electric field is
    % calculated from different probe pairs.
    
    start_time = data8ordfc(1,1);
    switch cl_id
        case 1 % CN: Cluster 1 spacecraft
