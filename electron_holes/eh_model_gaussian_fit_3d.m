...
%% Apply to MMS data, at z = 0, i.e. do not fit Epar, take into account epar also
ieh = 10; % Define which EH to use

% Use symbolic expressions to get derivative of phi = E
syms x y z phi0 lx ly lz angle x0 y0 z0 polar r
R = [x y z];
% Triple Gaussian potential structure
% need to take into account pitch angle
% something wrong here.

rotR = [sind(polar)*cosd(angle) sind(polar)*sind(angle)  cosd(polar);...
        cosd(polar)*cosd(angle) cosd(polar)*sind(angle) -sind(polar);...
...
%mf_xp = matlabFunction(xp,'Vars',[x y z angle x0 y0 z0 polar]); % 
%mf_yp = matlabFunction(yp,'Vars',[x y z angle x0 y0 z0 polar]); % V
%mf_zp = matlabFunction(zp,'Vars',[x y z angle x0 y0 z0 polar]); % V

% Order of the inputs defined by argument 'Vars'
mf_phi = matlabFunction(phi,'Vars',[x y z phi0 lx ly lz angle x0 y0 z0 polar]); % V
mf_Ex = matlabFunction(E(1),'Vars',[x y z phi0 lx ly lz angle x0 y0 z0 polar]); % mV/m (V/km)
mf_Ey = matlabFunction(E(2),'Vars',[x y z phi0 lx ly lz angle x0 y0 z0 polar]); % mV/m (V/km)
mf_Ez = matlabFunction(E(3),'Vars',[x y z phi0 lx ly lz angle x0 y0 z0 polar]); % mV/m (V/km)
...
phi0 = 400; % V
angle = 0;
polar = 10;manual(ieh).pitchangle;
...
params0 = double([phi0; lx0; ly0; lz0; angle; x0; y0; z0; polar]);
% I need to create a cost function, which is to be evaluated. For example
% ediffx = mf_Ex(vars) - Ex_obs
% ediffy = mf_Ey(vars) - Ey_obs
% ediffz = mf_Ez(vars) - Ez_obs
...
xvec = 2*linspace(-params(2),params(2),29); % km 
yvec = 2*linspace(-params(3),params(3),31); % km
...
if 1 % fit
  ...
  hca.YGrid = 'on';
  hold(hca,'on')
  c_eval('quiver(hca,r?(1,1),r?(1,2),Ex?_best,Ey?_best,''linewidth'',1,''color'',colors(?,:))',1:4)
  c_eval('plot(hca,r?(1,1),r?(1,2),''o'',''color'',colors(?,:))',1:4)
  hold(hca,'off')
  hca.XLabel.String = 'perp 1 (km)';
  hca.YLabel.String = 'perp 2 (km)';
end
...
%     [0.02 0.02],'color',[0 0 0])
  ...
  hleg = irf_legend(hca,{...
    sprintf('fitting parameters: \nphi0 = %.0f V \nlperp1 = %.1f km, lperp2 = %.1f km, lpar = %.1f km',params(1),params(2),params(3),params(4)),...    
    sprintf('angle = %.0f deg',params(5)),...
    sprintf('x0 (perp1) = %.0f km',params(6)),...
    sprintf('y0 (perp2) = %.0f km',params(7)),...
    sprintf('z0 (par) = %.0f km',params(8)),...  
    sprintf('polar = %.0f deg',params(9))}',...  
    [0.0 0.00],'color',[0 0 0]);
  hleg(1).VerticalAlignment = 'bottom';
end
