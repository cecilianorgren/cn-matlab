...
particles = eDist(1).macroparticles('ntot',1,'skipzero','scpot',scpot);
...
x_init = [X0(1),Y0(1),z0,0,0,-2.1e+05]; % small parallel speed
...
%  S(ie).T0 = units.me*VT(ie)^2/2/units.eV; % sqrt(t*units.eV*2/units.me)
  ...
  S(ie).vzstart = x_sol(1,6);
  S(ie).vzstop = x_sol(end,6);  
