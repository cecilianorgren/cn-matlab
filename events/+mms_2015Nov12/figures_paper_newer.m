...
colors = mms_colors('matlab');
c_eval('hmark_mp = irf_pl_mark(h(?),timeMP,0*colors(1,:)); hmark_mp.LineStyle = ''--''; hmark_mp.Color = [0.3 0.3 0.3];',1:npanels)
...
%% NEW FIGURES
%% Figure 3: 1 sc + 4sc grads, no DEFs
npanels = 8;
...
for ip = 5:8
  dy = 0.03;
  hca = h(ip);
  hca.Position(2) = hca.Position(2)-dy;
  h(5).Title.String = '4 spacecraft';
  h(4).XLabel.String = [];
end
%hca = irf_panel('B'); hca.YLim = [-14 14];
%hca = irf_panel('e PA e32 deflux all low E'); hca.CLim = [7.2 8.2];
...
legends = {'a)','b)','c)','d)','e)','f)','g)','h)','i)','j)','k)','l)','m)','n)','o)'};
%legends_color = {'k','k','k','k','w','w','w','k','k','k'};
legends_color = {'k','k','k','k','k','k','k','k','k','k'};
legshift = 0; % the two sc configuration plots
...
%% Figure 4: 1 sc more detailed, DEFs
npanels = 9;
tintZoom = irf.tint('2015-11-12T07:19:19.40Z/2015-11-12T07:19:23.10Z');
%tintZoom = irf.tint('2015-11-12T07:19:10.00Z/2015-11-12T07:19:40.00Z');
h = irf_plot(npanels);
...
if 1 % ePDist pa low energies
  ...
  if 1 % magnetic mirror angle
    ...
    c_eval('alphaB = irf.ts_scalar(mvaB?.tlim(tintB).time,asind((mvaB?.tlim(tintB).abs.data/mvaB?.abs.resample(tref).data).^0.5));',ic)
...
if 1 % Ve
  hca = irf_panel('Ve perp par 2'); isub = isub + 1; 
  set(hca,'ColorOrder',mms_colors('xyza'))
  c_eval('irf_plot(hca,{mvaVe?perp.x.tlim(tint),mvaVe?perp.y.tlim(tint),mvaVe?perp.z.tlim(tint),mvaVe?par.tlim(tint)},''comp'');',ic)
  %c_eval('irf_plot(hca,{gseVe?.x.tlim(tint),gseVe?.y.tlim(tint),gseVe?.z.tlim(tint)},''comp'');',ic)  
  hca.YLabel.String = {'v_e','(km/s)'};
...
if 1 % ePDist pa low energies
  hca = irf_panel('e PA e32 deflux all low E 2'); isub = isub + 1;  zoomPA = [zoomPA isub];
  elim = [10 500];
  c_eval('[~,hcb] = irf_spectrogram(hca,ePitch?.tlim(tintZoom+[-2 2]).elim(elim).deflux.specrec(''pa''),''log'');',ic)
...
for ip = 7:9
  ...
  hca = h(ip);
  hca.Position(2) = hca.Position(2)-dy;
  h(7).Title.String = sprintf('MMS %g',ic);
  h(6).XLabel.String = [];
end
hcb.Position(2) = hcb.Position(2)-dy;

%hca = irf_panel('B'); hca.YLim = [-14 14];
%hca = irf_panel('e PA e32 deflux all low E'); hca.CLim = [7.2 8.2];
%hca = irf_panel('e PA e32 deflux all mid E'); hca.CLim = [7.2 8.2];

h(1).Title.String = ['MMS' num2str(1)];

legends = {'a)','b)','c)','d)','e)','f)','g)','h)','i)','j)','k)','l)','m)','n)','o)'};
legends_color = {'k','k','k','w','w','w','k','k','k','k'};
legshift = 0; % the two sc configuration plots
