...
c_eval('[gseB?par,gseB?perp] = irf_dec_parperp(gseB?,gseB?); gseB?par.name = ''B par''; gseB?perp.name = ''B perp'';',ic)

ffilt = 0.1;
c_eval('dcB? = gseB?.filt(0,ffilt,[],3);',ic)
c_eval('acB? = gseB?-dcB1;',ic)
c_eval('phi?=(dcB?.abs*1e-9)*(acB?.abs*1e-9)/(ne?*1e6)/units.mu0/units.e;',ic)
...
% Save all time intervals in a structure array
% For wave disperion analysis
% These are/may be different for all time intervals
