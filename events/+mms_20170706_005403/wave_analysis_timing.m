...
c_eval('B? = gseB?.tlim(tint_vicinity);')
avB = [mean(B1.data,1); mean(B2.data,1); mean(B3.data,1); mean(B4.data,1)];
avB = mean(avB,1);
...
if 0 % integrate total E and dit with v
  ...
  c_eval('gsePhi? = gseEdt?*v_amplitude*double(sign(v_direction(1)))*double(sign(avB(1)));')
  c_eval('gsePhi?_detrend = gsePhi?; gsePhi?_detrend.data = detrend(gsePhi?_detrend.data,''linear'');')
end
...
if 0 % v phase + trap
  ...
  c_eval('vmin? = tsVphpar-tsVtrap?;',1:4)
  c_eval('vmax? = tsVphpar+tsVtrap?;',1:4)
  irf_patch(hca,{vmin1,vmax1})
  irf_patch(hca,{vmin2,vmax2})
  irf_patch(hca,{vmin3,vmax3})
  irf_patch(hca,{vmin4,vmax4})
