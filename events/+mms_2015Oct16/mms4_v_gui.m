...
switch action,
	case {'c1','c2','c3','c4','c5','c6'}
		...
		set(gcf,'userdata',ud);
		mms_2015Oct16.mms4_v_gui('update_var_col');
	case 'update_var_col'
		hca=ud.h(1);
		xl=get(ud.h(1),'XLim');
...
%		mms4_v_gui('dt');
	case 'new_var_enter'
		...
		set(gcf,'userdata',ud);
		mms_2015Oct16.mms4_v_gui('new_var');
	case 'new_var'
		...
		if flag_first_call,
			set(gcf,'userdata',ud);
			mms_2015Oct16.mms4_v_gui('initialize');
		else
			if ishandle(ud.h(1)),
				for j_col=2:size(ud.var1,2)
					if j_col<=length(ud.hcol),
						if ishandle(ud.hcol(j_col)),
							...
							eval_str=['ud.hcol(j_col)=uimenu(ud.columns,''label'',''' num2str(j_col) ''',''callback'',''mms_2015Oct16.mms4_v_gui(''''c' num2str(j_col) ''''')'');'];
							eval(eval_str);
						end
					else
						eval_str=['ud.hcol(j_col)=uimenu(ud.columns,''label'',''' num2str(j_col) ''',''callback'',''mms_2015Oct16.mms4_v_gui(''''c' num2str(j_col) ''''')'');'];
	...
	case 'initialize'
		...
		ud.dt_input = uicontrol('style', 'edit', ...
			'string', '[0 0 0 0]', ...
			'callback', 'mms_2015Oct16.mms4_v_gui(''dt'')', ...
		...
		ud.v = uicontrol('style', 'edit', ...
			'string', '0*[0 0 0]', ...
			'callback', 'mms_2015Oct16.mms4_v_gui(''v'')', ...
			'backgroundcolor','white','units','normalized','position', [xp+0.15 yp 0.29 0.05]);
		...
		uimenu('label','Auto &YLim','accelerator','y','callback','mms_2015Oct16.mms4_v_gui(''autoY'')');
		uimenu('label','&Distance','accelerator','d','callback','mms_2015Oct16.mms4_v_gui(''distance'')');
		uimenu('label','Click&Times','accelerator','t','callback','mms_2015Oct16.mms4_v_gui(''click_times'')');
		uimenu('label','New&Variable','accelerator','v','callback','mms_2015Oct16.mms4_v_gui(''new_var_enter'')');
	...
	case 'dt'
		...
		if max(abs(ud.dt))==0
			...
			v=mms_2015Oct16.mms4_v(t,coord_sys(ud));
	...
	case 'v'
		...
		if max(abs(v))==0
			...
			dt=mms_2015Oct16.mms4_v([t v],coord_sys(ud));
	...
	case 'click_times'
		...
		if ud.ic==0,
			set(gcf,'windowbuttondownfcn', 'mms_2015Oct16.mms4_v_gui(''click_times'')');
			ud.ic=1;
		else
