...
if 1 % ePDist pa 32 mms1
  ic = 1;
  hca = irf_panel(irf_ssub('e PA deflux, mms?',ic));
  c_eval('irf_spectrogram(hca,ePDist?.pitchangles(dmpaB?,[]).deflux.elim(eint).specrec(''pa''),''log'');',ic)
  %hca.YLabel.String = {'Pitchangle','(\circ)'};   
  hca.YLabel.String = {'\theta_{PA,e}','(\circ)'};   
  hca.YTick = [45 90 135];   
  hca.YLabel.String = {'\theta_{PA,e}',irf_ssub('MMS ?',ic), '(\circ)'};     
end
...
if 1 % Te
  hca = irf_panel('Te trace');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{mvaTe1.trace.tlim(tint),mvaTe2.trace.tlim(tint),mvaTe3.trace.tlim(tint),mvaTe4.trace.tlim(tint)},'comp');
  hca.YLabel.String = {'T_{e}','(eV)'};
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_legend(hca,{'mms 1','mms 2','mms 3','mms 4'},[0.98 0.9],'fontsize',12);
end
if 1 % Pe
  hca = irf_panel('Pe trace');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{mvaPe1.trace.tlim(tint),mvaPe2.trace.tlim(tint),mvaPe3.trace.tlim(tint),mvaPe4.trace.tlim(tint)},'comp');
  hca.YLabel.String = {'P_{e}','(nPa)'};
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_legend(hca,{'mms 1','mms 2','mms 3','mms 4'},[0.98 0.9],'fontsize',12);
end
if 0 % EN
