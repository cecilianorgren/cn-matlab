...
%% Delta B relation for LHW dynamics
c_eval('bdryB?dc = bdryB?.filt(0,0.1,[],5);',1:4)
c_eval('bdryB?ac = bdryB?-bdryB?dc;',1:4)
c_eval('bdryB?acdc = bdryB?ac/bdryB?dc;',1:4)
c_eval('ne?dc = ne?.filt(0,0.1,[],5);',1:4)
c_eval('ne?ac = ne?-ne?dc;',1:4)
c_eval('ne?acdc = ne?_ac/ne?dc;',1:4)
...
if 1 % B abs
  hca = irf_panel('B abs');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{bdryB1dc.abs.tlim(tint),bdryB2dc.abs.tlim(tint),bdryB3dc.abs.tlim(tint),bdryB4dc.abs.tlim(tint)},'comp');
  hca.YLabel.String = {'|B|_{DC}','(nT)'};
end
if 1 % B abs
  hca = irf_panel('B abs');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{bdryB1.abs.tlim(tint),bdryB2.abs.tlim(tint),bdryB3.abs.tlim(tint),bdryB4.abs.tlim(tint)},'comp');
  hca.YLabel.String = {'|B|_{AC}','(nT)'};
end
if 1 % B abs
  hca = irf_panel('B abs');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{bdryB1ac.abs.tlim(tint),bdryB2ac.abs.tlim(tint),bdryB3ac.abs.tlim(tint),bdryB4ac.abs.tlim(tint)},'comp');
  hca.YLabel.String = {'|B|_{AC}','(nT)'};
end
if 0 % BX
  hca = irf_panel('BX');
...
if 1 % ne
  hca = irf_panel('ne');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{ne1dc.tlim(tint),ne2dc.tlim(tint),ne3dc.tlim(tint),ne4dc.tlim(tint)},'comp');
  hca.YLabel.String = {irf_ssub('n_{e}',ic),'(cm^{-3})'};
end
if 1 % ne
  hca = irf_panel('ne');
