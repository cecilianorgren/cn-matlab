...
switch coordSystem % Choose another
  case 1 % N: minimum variance of B
    [out,l,v] = irf_minvar(gseB4.tlim(irf.tint('2015-11-19T10:41:18.669Z/2015-11-19T10:41:28.244Z')));
    L = v(1,:); M = v(2,:); N = v(3,:);
    coordLabels = {'L','M','N'};
    lmn = [N;-M;L];
  ...
  case 5 % N: magnetosphere side normal derived from mms3 and mms4
    ...
    M = M;
    L = cross(M,N);
  case 6 % N: minimum variance of B over large the bubble interval
    [out,l,v] = irf_minvar(gseB4.tlim(irf.tint('2015-11-19T10:40:29.731Z/2015-11-19T10:41:25.318Z')));
    L = v(1,:); M = v(2,:); N = v(3,:);
    coordLabels = {'L','M','N'};
    lmn = [N;-M;L];
end
...
tint = irf.tint('2015-11-19T10:40:04.00Z/2015-11-19T10:42:14.00Z');
...
if 1 % e DEF omni 64
  ...
  c_eval('[hout,hcb] = irf_spectrogram(hca,ePDist?.deflux.e64.omni.specrec,''log'');',ic)  
  set(hca,'yscale','log');
...
if 1 % ePDist pa 32
  eint = [20 1000];
  hca = irf_panel('e PA deflux E1');  
  c_eval('irf_spectrogram(hca,ePDist?.elim(eint).pitchangles(dmpaB?,24).deflux.specrec(''pa''),''log'');',ic)
...
%% Figure 2: Different comparison plot of more field components
tint = irf.tint('2015-11-19T10:41:00.00Z/2015-11-19T10:41:10.00Z');h = irf_plot(13);
if 1 % BL
  hca = irf_panel('BL');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{mvaB1.x.tlim(tint),mvaB2.x.tlim(tint),mvaB3.x.tlim(tint),mvaB4.x.tlim(tint)},'comp');
