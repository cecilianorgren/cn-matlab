...
%% Electron pitchangle at ring energy.
tint_zoom = irf.tint('2015-10-16T10:33:42.00Z/2015-10-16T10:33:52.00Z'); 
times_dist = EpochTT(['2015-10-16T10:33:44.507494873Z';...
              '2015-10-16T10:33:44.721328857Z';...
              '2015-10-16T10:33:45.001986328Z';...
              '2015-10-16T10:33:45.282643554Z';...
              '2015-10-16T10:33:45.549936035Z';...
  ]);
times_dist = EpochTT('2015-10-16T10:33:44.500Z'):0.1:EpochTT('2015-10-16T10:33:45.50Z');
nd = times_dist.length;
colors = colormap('jet'); pic_colors('thermal'); pic_colors('pasteljet');
colors = interp1(1:size(colors,1),colors,linspace(1,size(colors,1),nd))
%colors = colors(10:10:size(colors,1),:);
%interp1(linspace(1,101,size(c,1)),c,1:101);
%colors = interp1((1:size(colors,1))',pic_colors('thermal'),5:5:25');
elevel = 200;
elim = [180 250];
ic = 1;
[h,h2] = initialize_combined_plot(1,1,1,0.6,'vertical');
...
if 1 % eDist omni 64 perp
  ...
  %c_eval('ePitch?lim = ePDist?.pitchangles(dmpaB?,pas);',ic)  
  c_eval('ePitch?lim = ePitch?perp;',ic)  
  c_eval('[hout,hcb] = irf_spectrogram(hca,ePitch?lim.tlim(tint_zoom).deflux.specrec,''log'');',ic)  
  set(hca,'yscale','log');
  set(hca,'ytick',[1e1 1e2 1e3 1e4]);
  hca.YLim = [10,1000];
...
if 1
  ...
  holdon = 0;
  for it = 1:times_dist.length
    c_eval('epitch = ePitch?.resample(times_dist(it),''nearest'').elim(elim);',ic)
    %epitch.time
    plot(hca,epitch.depend{2},squeeze(epitch.data),'color',colors(it,:))
    if not(holdon)
      %1
      hold(hca,'on')
    end  
    legs{it} = irf_time(times_dist(it),'EpochTT>utc_HH:MM:SS.mmm');
  end
  hold(hca,'off')
  set(hca,'colororder',colors)
  irf_legend(hca,legs',[1.01 0.98])
  hca.XLim = [0 180];
end

hmark = irf_pl_mark(h,times_dist.epochUnix,'k');
