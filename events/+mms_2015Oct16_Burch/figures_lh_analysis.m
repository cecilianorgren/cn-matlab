...
%% Figure 1: 1 sc overview pressure balance, ion momentum balance 
%tint = irf.tint('2015-10-16T10:34:20.00Z/2015-10-16T10:35:30.00Z');
npanels = 7;
cmap = 'jet';
...
if 1 % Pressures
  ...
  set(hca,'ColorOrder',mms_colors('xyza'))
  %c_eval('irf_plot(hca,{gseVi?.x.tlim(tint),gseVi?.y.tlim(tint),gseVi?.z.tlim(tint),gseVi?.abs.tlim(tint)},''comp'');',ic)
  c_eval('irf_plot(hca,{PB?,gsePi?.trace/3,gsePe?.trace/3,gsePe?.resample(gsePi?).trace/3+gsePi?.trace/3+PB?.resample(gsePi?)},''comp'')',ic)
  hca.YLabel.String = {'P','(nPa)'};
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_legend(hca,{'P_B','P_i','P_e','P_B+P_i+P_e'},[0.98 0.9],'fontsize',12);  
end
if 1 % Vi
  hca = irf_panel('Vi');
  set(hca,'ColorOrder',mms_colors('xyza'))
...
if 0 % Ji moments 
  hca = irf_panel('J mom i');
  set(hca,'ColorOrder',mms_colors('xyz'))
  %c_eval('irf_plot(hca,{gseJcurl.x.tlim(tint),gseJcurl.y.tlim(tint),gseJcurl.z.tlim(tint),gseJcurl.abs.tlim(tint)},''comp'');',ic)
  c_eval('irf_plot(hca,{gseJi?.x.tlim(tint),gseJi?.y.tlim(tint),gseJi?.z.tlim(tint)},''comp'');',ic)
  hca.YLabel.String = {'J_i','(nA/m^2)'};
  set(hca,'ColorOrder',mms_colors('xyz'))
  irf_legend(hca,{'J_x','J_y','J_z'},[0.98 0.9],'fontsize',12);
  hca.YScale = 'lin';
end
if 0 % Je moments 
  hca = irf_panel('J mom e');
  set(hca,'ColorOrder',mms_colors('xyz'))
  %c_eval('irf_plot(hca,{gseJcurl.x.tlim(tint),gseJcurl.y.tlim(tint),gseJcurl.z.tlim(tint),gseJcurl.abs.tlim(tint)},''comp'');',ic)
  c_eval('irf_plot(hca,{gseJe?.x.tlim(tint),gseJe?.y.tlim(tint),gseJe?.z.tlim(tint)},''comp'');',ic)
  hca.YLabel.String = {'J_e','(nA/m^2)'};
  set(hca,'ColorOrder',mms_colors('xyz'))
  irf_legend(hca,{'J_x','J_y','J_z'},[0.98 0.9],'fontsize',12);
  hca.YScale = 'lin';
end
if 0 % Vi x
  hca = irf_panel('Vi x');
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_plot(hca,{gseAvVi.x,gseVDi.x,gseAvVExB.resample(gseAvVi).x,gseVDi.x+gseAvVExB.resample(gseAvVi).x},'comp');
  hca.YLabel.String = {'V_{ix}','(nT)'};
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_legend(hca,{'V_i','V_{Di}','V_{ExB}','V_{Di}+V_{ExB}'},[0.98 0.9],'fontsize',12);  
end
if 0 % Vi x
  hca = irf_panel('Vi y');
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_plot(hca,{gseAvVi.y,gseVDi.y,gseAvVExB.resample(gseAvVi).y,gseVDi.y+gseAvVExB.resample(gseAvVi).y},'comp');
  hca.YLabel.String = {'V_{iy}','(nT)'};
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_legend(hca,{'V_i','V_{Di}','V_{ExB}','V_{Di}+V_{ExB}'},[0.98 0.9],'fontsize',12);  
end
if 0 % Vi x
  hca = irf_panel('Vi z');
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_plot(hca,{gseAvVi.z,gseVDi.z,gseAvVExB.resample(gseAvVi).z,gseVDi.z+gseAvVExB.resample(gseAvVi).z},'comp');
  hca.YLabel.String = {'V_{iz}','(nT)'};
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_legend(hca,{'V_i','V_{Di}','V_{ExB}','V_{Di}+V_{ExB}'},[0.98 0.9],'fontsize',12);  
end
if 1 % gradPe
...
if 1 % gradPi
  hca = irf_panel('gradPi E');
  gseGradPi_filt = gseGradPi.filt(0,1,[],5);
  set(hca,'ColorOrder',mms_colors('xyz'))
  irf_plot(hca,{gseGradPi_filt.x*1e3,gseGradPi_filt.y*1e3,gseGradPi_filt.z*1e3},'comp');
...
if 0 % gradPi/ne
  hca = irf_panel('gradPi/ne');
  set(hca,'ColorOrder',mms_colors('xyz'))
  irf_plot(hca,{gseOhmGradPi.x,gseOhmGradPi.y,gseOhmGradPi.z},'comp');
  hca.YLabel.String = {'\nabla \cdot P_i/ne','(mV/m)'};
  %set(hca,'ColorOrder',mms_colors('xyz'))
  %irf_legend(hca,{'4 sc average'},[0.06 0.9],'fontsize',11,'color','k');
  set(hca,'ColorOrder',mms_colors('xyza')) 
  irf_legend(hca,{'x','y','z'},[0.98 0.9],'fontsize',12);
...
if 1 % Ohm's law x
  ...
  if 0
    ...
    irf_plot(hca,{gseAvE.resample(gseOhmVexB).x,gseOhmGradPe.x,-gseOhmVexB.x,-gseOhmVixB.x,gseOhmJxB.x},'comp');
    set(hca,'ColorOrder',mms_colors('1234ba'))
...
if 1 % Ohm's law y
  hca = irf_panel('Ohm Y');
  set(hca,'ColorOrder',mms_colors('1234b'))
  irf_plot(hca,{gseAvE.resample(gseOhmVexB).y,gseOhmGradPe.y,-gseOhmVexB.y,-gseOhmVixB.y,gseOhmJxB.y},'comp');
...
if 1 % Ohm's law z
  hca = irf_panel('Ohm Z');
  set(hca,'ColorOrder',mms_colors('1234b'))
  irf_plot(hca,{gseAvE.resample(gseOhmVexB).z,gseOhmGradPe.z,-gseOhmVexB.z,-gseOhmVixB.z,gseOhmJxB.z},'comp');
...
if 1 % Ohm's law residual 3 components
  ...
  set(hca,'ColorOrder',mms_colors('xyz'))
  residual = gseAvE.resample(gseOhmVexB)+gseOhmVexB+gseOhmGradPe;
