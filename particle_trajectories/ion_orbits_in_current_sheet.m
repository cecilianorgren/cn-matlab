...
Ez = @(x,y,z) x*0;
Ez = @(x,y,z) 8*E0*-1*(z/1/lz).*exp(-(z/(lz)).^2);
...
if 1 % Figure: (x,z) (x,y)
  ...
  iPs = 1:nP; 
    ...
    [X,Z] = ndgrid(xx,zz);
    AY = Ay(X,Z*0,Z,0);
    contour(hca,X*1e-3,Z*1e-3,AY,10,'k')
    %hp = pcolor(hca,X*1e-3,Z*1e-3,Bx(X*0,X,Z));
    %hp.FaceAlpha = 1;
    %shading(hca,'flat')
    ...
    hold(hca,'on')
    for ip = iPs
      plot(hca,p(ip).x*1e-3,p(ip).z*1e-3,'LineWidth',2)
    end
    hold(hca,'off')
    ...
    if 0 % E quivers
      ...
      ex = Ex(Y*0,Y,Z);
      ez = Ez(Y*0,Y,Z);
      hq = quiver(hca,X*1e-3,Z*1e-3,ex*1e3,ez*1e3,0,'color','k','marker','.','MaxHeadSize',0);
      hold(hca,'off'),
    end
