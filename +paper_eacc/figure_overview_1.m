...
if 0 % Jx, moments and curl  iisub = iisub + 1;
  ...
  set(hca,'ColorOrder',mms_colors('x2'))
...
legends_color = {'w','k','k','k','k','k','k','k','k','k','k','k','k','k','k','k','k'};
nInd = 1;
for ii = [1:npanels_large npanels_large + 1 + [1:npanels_zoom]]  
  irf_legend(h(ii),legends{nInd},[0.01 0.95],'color',legends_color{ii},'fontsize',14)
  nInd = nInd + 1;
end

c_eval('h(?).Position(2) = h(?).Position(2) + 0.05;',(npanels_large+2):npanels)

irf_zoom(h(1:npanels_large),'x',tint_overview)
irf_zoom(h(npanels_large+1+[1:npanels_zoom]),'x',tint_waves_zoomin)
irf_zoom(h(zoomy),'y') 
irf_plot_axis_align
...
h(npanels_large).XLabel.String = '';
%hca = irf_panel('Jx fpi fgm'); hca.YLim = [-70 70];
