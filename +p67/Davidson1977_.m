...
ne = n;
ni = n;
Te = 5; Te_K = Te*units.eV/units.kB;
Ti = 0.01; Ti_K = Ti*units.eV/units.kB;
B = 40e-9;
...
wce = qe*B/me; fce = wce/2/pi;
wci = qe*B/mi; fci = wci/2/pi;
roe = vte/wce;
roi = vti/wci;
Ld = sqrt(vte^2)/wpe/sqrt(2)*1e-3;

wlh = sqrt(wce*wci); flh = wlh/2/pi;
% Gradient length scales
Ln = 50e3; % 10 km; % 1/(2e-5);5*roe;10e3;
LB = 10e5;
LT = 10e5;
...
nk = 80;
kvec = 0.6*linspace(1e-3,1,nk)/roe;
dk = kvec(2)-kvec(1);
...
wnorm = wlh; wpi;
plot(h(1),kvec(1)*roe,wr_(1)/wnorm,'b.',kvec(1)*roe,wi_(1)/wnorm,'r.'); hold(gca,'on')
h(1).YLabel.String = '\omega/\omega_{LH}';
h(1).XLabel.String = 'k\rho_e';
titleString = sprintf('T_e = %d eV, T_i = %g eV, B = %g nT',Te,Ti,B*1e9); 
h(1).Title.String = titleString;

plot(h(1),[0 kvec(end)*roe])
...
for nn = 2:length(kvec)
  frange = vguess*dk*2;
  if nn>3; frange = vguess*dk*3; end
  ...
  if 1
    plot(h(1),kvec(nn)*roe,wr_(nn)/wnorm,'b.',kvec(nn)*roe,wi_(nn)/wnorm,'r.')
    %errorbar(h(1),kvec(nn)*roe,wrv(fix(nf/2))/wnorm,frange/wnorm)
    %errorbar(h(1),kvec(nn)*roe,wiv(fix(nf/2))/wnorm,frange/wnorm)
    plot(h(2),kvec(nn)*roe,min(min(z)),'k.')
    h(2).YLabel.String = 'residue';
    h(2).XLabel.String = 'k\rho_e';
    h(2).YScale = 'log';
  else
    pcolor(abs(z)); shading flat;
    colorbar
  ...
  disp(sprintf('kre = %0.2f, gradr/wlh = %g, gradi/wlh = %g, residue = %g',kvec(nn)*roe,gradr/wnorm,gradi/wnorm,min(min(z))))
end
