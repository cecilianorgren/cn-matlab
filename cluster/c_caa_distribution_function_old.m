...
switch plot_type
    case 'polar' % Plot polar distribution
        switch n_products % Prepare products
            case 1 % Only one product that will be plotted on both sides.
                product=products{1};
                ...
                if any(strfind(product,'PITCH'))
                    ...
                    X = r_man*cosd(theta); % x-coordinates
                    Y = r_man*sind(theta); % y-coordinates
                    
                    data_0=find(specrec.data==0);
                    specrec.data(data_0)=NaN;
                    
                    resultat=log10(specrec.data(:,:,:));
                    C=nanmean(resultat,1); % take mean of all values
                    ...
                    if 0
                    surf(ax,X,Y,X*0,CC); hold(ax,'on')
                    surf(ax,-flipdim(X,2),Y,X*0,CC); 
                ...
                if any([strfind(product,'3DXPH'),...
                    ...
                    if strfind(product,'3DXPH'); 
                        ...
                        [caaSEDL,~,SEDL]=c_caa_var_get(['delta_plus_energy_table__', product]);
                        [caaSEDU,~,SEDU]=c_caa_var_get(['delta_minus_energy_table__', product]);                      
                    end
                    ...
                    specrec.f=[specrec.f-dtheta specrec.f(end)+dtheta];
                    specrec.f_label='Pitch angle';
                    ...
                    %surf(ax,Xplot,Yplot,Xplot*0,Cplot(1:end,:));
                    %view(2); axis(ax,'equal','tight'); shading(ax,'flat'); grid(ax,'off');
                    %cb=colorbar;
                    %ylabel(cb,specrec.p_label)
                ...
                Yplot{2}=Y;
                Cplot{2}=CC;
            case 2 % Two different products plotted in opposite sides.
                if 0
                product1=products{1};
                product2=products{2};
                ...
                for k=1:n_products;
                    ...
                    if strfind(products{k},'3DXPH'); 
                        ...
                        r0=emin-r_man(1);
                        r_man=rlog;
                        theta = double(specrec.f)+90; % pitch angles
        ...
        % Plot data
        surf(ax,Xplot{1},Yplot{1},Xplot{1}*0,Cplot{1}); hold(ax,'on');
        surf(ax,-flipdim(Xplot{2},2),Yplot{2},Xplot{2}*0,Cplot{2}); 
