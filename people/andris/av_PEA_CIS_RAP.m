...
tint=[toepoch([2007 8 31 10 00 0]) toepoch([2007 8 31 12 00 0])];
...
if 1, % plot panels
    ...
    if 1,    % SUBPLOT: PEACE energy spectrogram
        hca=h(i_subplot); i_subplot=i_subplot+1;
        irf_plot(hca,'Data__C4_CP_PEA_PITCH_SPIN_DEFlux','sum_dim1','colorbarlabel','log10 dEF\newline keV/cm^2 s sr keV','fitcolorbarlabel');
        caxis([5.9 7.6]);
    ...
    if 1,    % SUBPLOT: RAPID electron spectrogram
        hca=h(i_subplot); i_subplot=i_subplot+1;
        irf_plot(hca,'Electron_Dif_flux__C4_CP_RAP_ESPCT6','colorbarlabel','log10 dF\newline 1/cm^2 s sr keV','fitcolorbarlabel');
        caxis([0.51 4.49]);
    ...
    irf_timeaxis(h);
if 0,         % TMARKS, mark time intervals
