...
% remove background
%[eDist_new] = mms.remove_edist_background(eDist);
...
tic; ef1D = eDist.reduce('1D',dir_red,'scpot',scpot,'lowerelim',lowerelim,'nMC',nMC,'vg',vg); toc % reduced distribution along B
...
if 0
  %%
  fsheet_orig = ef1D.tlim(tint_sheet);
  fsheet_nobg = ef1D_new.tlim(tint_sheet);
...
hca = subplot(2,1,1);
plot(hca,plot_x,ef1D.data(25:1:end-10,:)')
hca.XLim = [0 5];
hca.YLim = [0 1]*1e-3;
%hca.XScale = 'log';
hca = subplot(2,1,2);
plot(hca,ef1D.depend{1}(1,:)*1e-3,ef1D.data(10:1:end-10,:)')
