...
c_eval('nvve? = gsePe?.data*0;',ic);
c_eval('nvvi? = gsePi?.data*0;',ic);
for ic1 = 1:3
  for ic2 = 1:3
    c_eval('nvve?(:,ic1,ic2) = units.me*1e6*1e3*1e3*ne?.data.*gseVe?.data(:,ic1).*gseVe?.data(:,ic2);',ic);
...
if 1 % Ve
  ...
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_legend(hca,{['v_' comps(1)],['v_' comps(2)],['v_' comps(3)],'-v_{te}'},[0.98,0.3],'fontsize',fontsize);
end
if 1 % NVVe-off LMN
...
if 1 % Se, Pe, nvv, xy
  ...
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_legend(hca,{'S_{xy}','P_{xy}','m_env_xv_y'},[0.98 0.15],'fontsize',fontsize_leg);
  %irf_legend(hca,{'LM','LN','MN'}',[1.02 0.9],'fontsize',fontsize);
end
if 0 % NVVe-off LMN
  hca = irf_panel('mnvve off LMN');
...
if 1 % gradients, comparison
  ...
  irf_legend(hca,{'\partial_t{S_{xy}}','\partial_t{P_{xy}}'},[0.02 0.15],'fontsize',fontsize);
  set(hca,'ColorOrder',mms_colors('z'))
  irf_legend(hca,{'m_e\partial_t(nv_xv_y)'},[0.98 0.98],'fontsize',fontsize_leg);  
  set(hca,'ColorOrder',mms_colors('ab'))
  irf_legend(hca,{'m_env_x\partial_t{v_y}'},[0.98 0.35],'fontsize',fontsize_leg);
  set(hca,'ColorOrder',mms_colors('b'))
  irf_legend(hca,{'m_ev_y\partial_t(nv_x)'},[0.98 0.15],'fontsize',fontsize_leg);
...
if 1 % gradients, comparison, mV/m  
  ...
  %irf_legend(hca,{'\partial_x{S_{xy}}','\partial_x{P_{xy}}','\partial_x(nv_x*v_y)','n*vx*\partial_x{v_y}','v_y\partial(nv_x)'},leg_loc,'fontsize',fontsize);
  set(hca,'ColorOrder',mms_colors('xy'))
  irf_legend(hca,{'-\partial_x{S_{xy}}/ne','-\partial_x{P_{xy}}/ne'},[0.02 leg_loc(2)],'fontsize',fontsize_leg);
  set(hca,'ColorOrder',mms_colors('z'))
  irf_legend(hca,{'-m_e\partial_x(nv_xv_y)/ne'},[0.98 0.98],'fontsize',fontsize_leg);  
  set(hca,'ColorOrder',mms_colors('ab'))
  %irf_legend(hca,{'-m_env_x\partial_x{v_y}/ne','-m_ev_y\partial_x(nv_x)/ne'},leg_loc,'fontsize',fontsize_leg);
  irf_legend(hca,{'-m_env_x\partial_x{v_y}/ne'},[0.98 0.75],'fontsize',fontsize_leg);
  set(hca,'ColorOrder',mms_colors('b'))
  irf_legend(hca,{'-m_ev_y\partial_x(nv_x)/ne'},[0.98 0.15],'fontsize',fontsize_leg);
  %irf_legend(hca,{sprintf('N_{smooth} = %g',nsm)},[0.5 0.99],'fontsize',fontsize,'color','k');
...
if 1 % n*Ve, grad_x(nVe) with two yaxes
  ...
  %ax2.Visible = 'off';
  ax2.YLabel.String = {'\partial_x(nv_{ex}) (cm^{-3}s^{-1})'};
  
  set(hca,'ColorOrder',[mms_colors('1');color_ax])
  irf_legend(hca,{'nv_{ex}','\partial_x(nv_{ex})'},[0.98 0.95],'fontsize',fontsize_leg);
  %set(gcf,'defaultAxesColorOrder',[0 0 0; 0 0 0]);
  ax2.YLim = [-29 39]*4/39;
end
...
if 1 % S
  ...
  hca.YLabel.String = ['\Pi',sprintf('_{e%s} (pPa)',comps)];
...
if 1 % make TSeries of integrated dP, dS
  ...
  h1(1).YLabel.String = ['\Pi',sprintf('_{e%s} (pPa)',comps)];
  h1(1).YLabel.Interpreter = 'tex';
  h1(2).YLabel.Interpreter = 'tex';
end
...
for itime = 1:times.length
  ...
  if 1 % change units
    ...
    hc_.Colorbar.YLabel.String = {['d\Pi',sprintf('_{e%s%s}(v_x,v_y)',comps(1),comps(2))],'(pPa)'};
    hca.CLim = dp_clim;
    1;    
  end
  if 1 % plot B direction
