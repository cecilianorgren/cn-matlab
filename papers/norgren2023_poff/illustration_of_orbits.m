...
a = 5;
b = 1;
xvec = a*linspace(-10.1,10,500)*1e3;
zvec = b*linspace(-10,10,100)*1e3;
yvec = linspace(-0,0,1);
...
if 1 % Figure 2
  ...
  Alev = [-105:10:110]*B0;
  Axline = ([0 0]-0.001)*B0;
  S = contourcs(xvec,zvec,squeeze(Ay(X,Y,Z))');
  SX = contourcs(xvec,zvec,squeeze(Ay(X,Y,Z))',Axline); 
  %[C,H] = contour3()
  nrows = 2;
  ncols = 1;
  clear h
...
By = @(x,y,z) -1e1*(z/2/b).*exp(-(z/(2*b)).^2).*(x/3/a).*exp(-(x/(3*a)).^2);
Bz = @(x,y,z) 2*x/a^2;
Ex = @(x,y,z) x*0;
Ey = @(x,y,z) x*0 + 0.2;
Ez = @(x,y,z) x*0;
...
if 1 % Figure 2
  ...
  if 1 % (vy,vz)
    ...
    for ip = 1:numel(p) % Particle trajectories
      % Forces
      by = By(p(ip).x,p(ip).y,p(ip).z);
      qvzBy = -q*p(ip).vz.*by;
      qvxBy = q*p(ip).vx.*by;
      hs = scatter(hca,p(ip).vy,p(ip).vz,30,qvxBy,'filled');
      colormap(hca,pic_colors('blue_red'))
      
      % Trajectory
      plot(hca,p(ip).vy(1),p(ip).vz(1),'Marker','o','color',colors(ip,:))
      plot(hca,p(ip).vy(end),p(ip).vz(end),'Marker','x','color',colors(ip,:))
      plot(hca,p(ip).vy,p(ip).vz,'color',colors(ip,:),'linewidth',0.5)
