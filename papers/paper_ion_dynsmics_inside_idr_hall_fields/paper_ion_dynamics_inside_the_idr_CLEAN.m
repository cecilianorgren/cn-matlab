...
c_eval('gseJzBx? =    gseJ?.z*gseB?.x.resample(gseJ?); gseJzBx?.name = ''Jz*Bx''; gseJxBy?.units = ''nA/m^2 nT'';',ic)
c_eval('gseJxBz? = -1*gseJ?.x*gseB?.z.resample(gseJ?); gseJxBz?.name = ''-Jx*Bz''; gseJxBz?.units = ''nA/m^2 nT'';',ic)
...
%PD_diff_notmasked = PD_clean_diff;

PD_orig_mask = PD_orig.mask('energy','mat',matEmask);
PD_clean_mask = PD_clean.mask('energy','mat',matEmask);
%PD_diff_mask = PD_clean_diff.mask('energy','mat',matEmask);

PD_clean_diff = PD_orig+-PD_clean.nan2zero; % 3D/time method
PD_mask_diff = PD_orig +- PD_orig_mask.nan2zero; % energy/time method
PD_used_diff = PD_orig +- PD_clean_mask.nan2zero;


h = irf_plot({PD_orig.deflux.omni.specrec,...
  PD_orig_mask.deflux.omni.specrec,...
  PD_mask_diff.deflux.omni.specrec,...
  PD_clean_notmasked.deflux.omni.specrec,...
  PD_clean_diff.deflux.omni.specrec,...
  PD_clean_mask.deflux.omni.specrec,...
  PD_diff_used.deflux.omni.specrec}); 
colormap(irf_colormap('magma'))
c_eval('h(?).YScale = ''log'';',1:numel(h))
hlinks = linkprop(h,{'CLim','YLim'});
c_eval('hb(?) = colorbar(h(?)); hb(?).YLabel.String = {"DEF","keV/(cm^2 s sr keV)"}; hb(?).FontSize = 7;',1:numel(h)) 
c_eval('h(?).YLabel.String = ''E (eV)'';',1:numel(h)) 
c_eval('h(?).YLabel.FontSize = 15;',1:numel(h)) 
c_eval('h(?).FontSize = 15;',1:numel(h)) 
h(end).XTickLabelRotation = 0;
fontsize_text = 13;
irf_legend(h(1),{'Original'},[0.02 0.04],'color','w','fontsize',fontsize_text)
irf_legend(h(2),{'Cleaned based on energy/time structure'},[0.02 0.04],'color','k','fontsize',fontsize_text)
irf_legend(h(3),{'Original - (Cleaned based on energy/time structure)'},[0.02 0.04],'color','w','fontsize',fontsize_text)
irf_legend(h(4),{'Cleaned based on 3D/time structure'},[0.02 0.04],'color','w','fontsize',fontsize_text)
irf_legend(h(5),{'Original - (Cleaned based on 3D/time structure)'},[0.02 0.04],'color','w','fontsize',fontsize_text)
irf_legend(h(6),{'Cleaned based on 3D/time and energy/time structure'},[0.02 0.04],'color','k','fontsize',fontsize_text)
irf_legend(h(7),{'Original - (Cleaned based on 3D/time and energy/time structure)'},[0.02 0.04],'color','w','fontsize',fontsize_text)
%%


PD_orig = PD_orig_mask;
PD_clean = PD_clean_mask;
PD_clean_diff = PD_diff_mask;
%PD_clean = PD_orig;
...
lmn_vi = [L_vi; M_vi; N_vi];
 ...
 % Torbert 2018
L_tor = [0.971, 0.216, -0.1069];
M_tor = [-0.234, 0.948, -0.215]; 
N_tor = [0.054, 0.233, 0.971];
lmn_tor = [L_tor; M_tor; N_tor];
...
%lmn = lmn_edr;
lmn = lmn_eig;
lmn = lmn_tor; % for referee response
lmn = lmn_edr; % for referee response, edr = nakamura
lmn = lmn_eig2;
L = lmn(1,:);
M = lmn(2,:);
N = lmn(3,:);
...
%%
c_eval('mvaJ? = gseJ?*lmn'';',ic)
c_eval('mvaJxBne?_mVm = gseJxBne?_mVm*lmn'';',ic)
...
if 1 % Vi
  ...
  hold(hca,'on')
  %c_eval('irf_plot(hca,{mvaVExB?.resample(mvaVi?).x.tlim(tint),mvaVExB?.resample(mvaVi?).y.tlim(tint),mvaVExB?.resample(mvaVi?).z.tlim(tint)},''comp'');',ic)  
  hold(hca,'off')
...
if 1 % E lmn
  ...
  set(hca,'ColorOrder',mms_colors('xyza'))  
  fhighE = 10;
  c_eval('irf_plot(hca,{mvaE?.filt(0,fhighE,[],5).x,mvaE?.filt(0,fhighE,[],5).y,mvaE?.filt(0,fhighE,[],5).z,mvaVixB?.z},''comp'');',ic)
  %hold(hca,'on')
  %c_eval('irf_plot(hca,{mvaVixB?.z},''comp'');',ic)
  %hold(hca,'off')
  hca.YLabel.String = {'E','(mV/m)'};
  set(hca,'ColorOrder',mms_colors('xyza'))
  irf_legend(hca,{'L','M','N'},[0.98 0.98],'fontsize',fontsize);
  irf_legend(hca,{sprintf('< %g Hz',fhighE)},[0.98 0.1],'fontsize',fontsize-2,'color','k');
end 
...
if 1 % dEFlux ion
  ...
  [h_sp,hcb] = irf_spectrogram(hca,specrec,'donotfitcolorbarlabel');
  hca.YScale = 'log'; 
  hcb.YLabel.String = 'DEF keV/(cm^2 s sr keV)';
...
if 1 % fi red L
  ...
  c_eval('[~,hcb] = irf_spectrogram(hca,specrec,''donotfitcolorbarlabel'');',ic)   
  delete(hcb)
  set(hca,'ColorOrder',mms_colors('xyza'))
...
hlinks = linkprop(h(1+[5 6 7]),{'CLim'});
h(1+6).CLim = [-4   -1];
...
if 1 % JxB/ne mV/m, N
  ...
  c_eval('irf_plot(hca,{mvaE?.z.filt(0,fhighE,[],5),mvaJxBne?_mVm.z.filt(0,fhighE,[],5),mvaJLBM?ne_mVm.filt(0,fhighE,[],5),mvaJMBL?ne_mVm.filt(0,fhighE,[],5)},''comp'');',ic)
...
if 1 % JxB/ne mV/m
  hca = irf_panel('E Hall LMN L');h
  set(hca,'ColorOrder',mms_colors('1243b'))
  %c_eval('irf_plot(hca,{mvaE?.x.filt(0,fhighE,[],5),mvaJxBne?_mVm.x,mvaJNBM?ne_mVm,mvaJMBN?ne_mVm},''comp'');',ic)
  c_eval('irf_plot(hca,{mvaE?.x.filt(0,fhighE,[],5),mvaJxBne?_mVm.x.filt(0,fhighE,[],5),mvaJNBM?ne_mVm.filt(0,fhighE,[],5),mvaJMBN?ne_mVm.filt(0,fhighE,[],5)},''comp'');',ic)
  
  hca.YLabel.String = {'E_L (mV/m)'};
  set(hca,'ColorOrder',mms_colors('1243b'))
  irf_legend(hca,{,sprintf('E (< %g Hz)',fhighE),'JxB/ne','-J_NB_M/ne','J_MB_N/ne'},[0.98,0.98],'fontsize',fontsize);
end

if 0 % JxB/ne mV/m
  hca = irf_panel('E Hall LMN M');
  set(hca,'ColorOrder',mms_colors('1243b'))
  %c_eval('irf_plot(hca,{mvaE?.x.filt(0,fhighE,[],5),mvaJxBne?_mVm.x,mvaJNBM?ne_mVm,mvaJMBN?ne_mVm},''comp'');',ic)
  c_eval('irf_plot(hca,{mvaE?.y.filt(0,fhighE,[],5),mvaJxBne?_mVm.y,mvaJNBL?ne_mVm,mvaJLBN?ne_mVm},''comp'');',ic)
...
if 1 % iAzim_170
  ...
  %ts = iAzim.mask('energy','max',tsElow);
  ts = ts.elim(elim);
  %ts = iAzim;
  ...
  %hca.CLim = [-30 -26]+30;
  %hca.CLim = [1 4];
  if 1 % make second axis with injection angle
    ax2 = axes('Position',hca.Position,'YAxisLocation','right','color','none','TickDir','out');
    ax2.YLim = hca.YLim;
    ax2.YTick = hca.YTick;
    old_ticks = ax2.YTick;
    new_ticks = old_ticks-90;
    new_ticks(new_ticks<-90) = new_ticks(new_ticks<-90)+180;
    new_ticks = abs(new_ticks);
    new_labels = arrayfun(@(x) sprintf('%g',x), new_ticks, 'UniformOutput', false);
    ax2.YTickLabel = new_labels;
    ax2.YLabel.String = '\theta_{inj}';  
    ax2.XTick = [];
1;
%hcb.Location = 'north';
%hcb.Position(1) = 0.8;
%hcb.Position(3) = 0.08;
    hcb.Position(1) = 0.925;
    hcb.Position(2) = 0.55;
    hcb.Position(4) = 0.2;
  end
end
    ...
    if 1 % add elim circular markings and angular marking combined
      ...
      for ia = 1:numel(angs)
        ...
        if angs(ia) == 120
          ...
          ht_ = text(hca,xx(2)*text_margin-1000,yy(2)*text_margin-450,{'\theta_{LN}='}, ...
...
c_eval('h1(?).XGrid = ''off''; h1(?).YGrid = ''off''; h1(?).Layer = ''top'';',1:numel(h1))
...
irf_legend(hca,{sprintf(['L'],vL_Xline),'  M','   N'},[0.9 0.08],'fontsize',fontsize);
...
for ip = numel(h)-1%3:nRows:numel(h)
  vmax = -2000;
...
%
irf_legend(h1(1),{'A'},[0.465 1.05],'color','k','fontsize',fontsize,'horizontalalignment','center','fontweight','bold')
irf_legend(h1(1),{'B'},[0.580 1.05],'color','k','fontsize',fontsize,'horizontalalignment','center','fontweight','bold')
irf_legend(h1(1),{'C'},[0.630 1.05],'color','k','fontsize',fontsize,'horizontalalignment','center','fontweight','bold')
irf_legend(h1(1),{'D'},[0.680 1.05],'color','k','fontsize',fontsize,'horizontalalignment','center','fontweight','bold')
irf_legend(h1(1),{'E'},[0.765 1.05],'color','k','fontsize',fontsize,'horizontalalignment','center','fontweight','bold')
irf_legend(h1(1),{'F'},[0.960 1.05],'color','k','fontsize',fontsize,'horizontalalignment','center','fontweight','bold')
%
...
%%
%%
%% Figure, overplot different times as patches
fontsize = 12;
