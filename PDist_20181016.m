...
%     userData = [];
  ...
  methods
    ...
    function PD = vd3v(obj,varargin)
      ...
      % int(v^3dv) -> v^4/4
      if doScpot
        %E = obj.depend{1} - repmat(scpot.data,1,size(obj.depend{1},2));
        E_minus = (obj.depend{1} - obj.ancillary.delta_energy_minus) - repmat(scpot.data,1,size(obj.depend{1},2));
        E_plus = (obj.depend{1} + obj.ancillary.delta_energy_plus)   - repmat(scpot.data,1,size(obj.depend{1},2));      
        E_minus(E_minus<0)= 0;
        E_plus(E_plus<0)= 0;
      ...
      vd3v = d_vel_mat.*d_azim.*d_polar_mat;  
      
      old_units = obj.units;
      switch obj.units
        case 's^3/cm^6' 
          % m^4/s^4 = m^4/s^4 * cm^4/cm^4 = cm^4/s^4 * m^4/cm^4 = cm^4/s^4 * (10^-2)^4
          d3v_scale = 1/10^(-2*4);
          new_units = '1/cm^2s';
        case 's^3/m^6'  
          % m^4/s^4 = m^4/s^4 * m^4/m^4 = m^4/s^4 * m^4/m^4 = m^4/s^4 * (10^0)^4
          d3v_scale = 1/10^0;
          new_units = '1/m^2s';
        case 's^3/km^6'  
          % m^4/s^4 = m^4/s^4 * km^4/km^4 = km^4/s^4 * m^4/km^4 = km^4/s^4 * (10^3)^4
          d3v_scale = 1/10^(3*4);
          new_units = '1/km^2s';
      end
    ...
    function PD = pa_flux(obj,varargin)
      % int(v f d3v) performed on pitchangle distribution
...
%             doCirclesDrifting = 1;
      ...
      if doScpot, rho_edges = rho_edges - scpot; rho_edges(rho_edges<0) = 0; data(rho_edges == NaN,:) = 0; end
