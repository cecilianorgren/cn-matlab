...
if 1 % plot, timeseries, for paper
  ...
  if 1 % F
    ...
    if 1 % model phase velocity
      hold(hca,'on')
      line_color = [0.5 0.5 0.5]; %line_color = mms_colors('matlab');
      %hlines = irf_plot(hca,tsVph*1e-6,'LineWidth',1.5,'Color',line_color(1,:),'LineStyle','-.');
      hlines = irf_plot(hca,tsVph*1e-6,'--k');
      irf_legend(hca,{'-- v_{ph,mod}'},[0.2 0.99],'color',hlines(1).Color);  
      hold(hca,'off')
    end
    if 1 % observed phase velocity
      hold(hca,'on')
      hlines = irf_plot(hca,obs_vph*1e-3,'*k','LineWidth',1.5,'Color',[0 0 0]);
      irf_legend(hca,{'* v_{ph,obs}'},[0.1 0.99],'color',hlines(1).Color);  
      hold(hca,'off')
    end  
    colormap(hca,cn.cmap('white_blue'))
    if 0 % str info
