...
xlims=[0 5];ylims=[8.5 10.5];zlims=[0 10];
...
for k=1:3
    figure; h=irf_plot(9);
    for l=1:9
        irf_plot(h(l),[(1:17531)' squeeze(Epar(m,(k-1)*9+l,:,:))]);
        eval(['ylabel(h(l),''',num2str(uniquecores(m)),'/',num2str((k-1)*9+l),''')']);
    end
end
end
...
for k=1:size(B,1)
    for p=1:size(B,2)
        Bnorm(k,p,:,:)=irf_norm(squeeze(B(k,p,:,:)));
    end
end
...
for k=1:length(data{1})   
    cor=data{1}(k);
    sat=data{2}(k);     
    quiver(Position(cor,sat,1),Position(cor,sat,2),Position(cor,sat,3),Bnorm(1),Bnorm(2),Bnorm(3)); hold on;
