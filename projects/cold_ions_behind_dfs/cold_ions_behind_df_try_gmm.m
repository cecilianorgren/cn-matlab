...
for iDF = iDFs%87%iDFs(1)
  ...
  for it = 1:nt
    ...
    for nGroups = 1:nGroupsMax
      ...
      S.Sigma = repmat([50000 10 10; 10 50000 10; 10 10 50000],[1 1 nGroups]);  
  ...
  % Get the temperature of each component 
%  vt2 = gm.Sigma*1e6; % (km/s)^2 -> (m/s)^2
...
%  end
  ...
  compact_panels(h2,0.05,0.01,1)
  drawnow
  if doPrint 
    cn.print(sprintf('gmm_vdf_df_id%04.0f_t0_%s',iDF,time.utc('yyyymmdd_HHMMSS')))
  end
  catch
    disp('Skipping.')
  end
