...
%     userData = [];
  ...
  methods
    ...
    function PD = reduce(obj,dim,x,varargin) 
      ...
      emat_delta_energy_plus = dist.ancillary.delta_energy_plus;
      if correct4scpot
        scpot = scpot.tlim(dist.time).resample(dist.time,'max'); % take maximum spacecraft potential during the interval
        scpot_mat = repmat(scpot.data, size(emat(1,:)));
        [it_below_scpot,ie_below_scpot] = find(emat + emat_delta_energy_plus*0 < scpot_mat); % use upper part of energy interval as limit
      ...
      for i = 1:nt
        ...
        if correct4scpot
          if 0
            ...
            ie_below_scpot = find(abs(emat(it(i),:)+emat_delta_energy_plus(it(i),:)-scpot_mat(it(i),:))); % energy channel below 
            ie_below_scpot = find(abs(emat(it(i),:)+emat_delta_energy_plus(it(i),:)-scpot_mat(it(i),:)) == min(abs(emat(it(i),:)+emat_delta_energy_plus(it(i),:)-scpot_mat(it(i),:)))); % closest energy channel
            remove_extra_ind = 3; % for margin, remove extra energy channels
