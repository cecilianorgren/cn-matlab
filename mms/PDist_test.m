...
[h_surf,h_ax,h_all] = ePDist1(490).pitchangles(dmpaB1,15).plot_pad_polar(hca);

hca = h(isub); isub = isub + 1;
[h_surf,h_ax] = ePDist1(490).pitchangles(dmpaB1,15).plot_pad_polar(hca);

hca = h(isub); isub = isub + 1;
[h_surf] = ePDist1(490).pitchangles(dmpaB1,15).plot_pad_polar(hca);

hca = h(isub); isub = isub + 1;
[h_surf,h_ax,h_all] = ePDist1(490).reduce('2D',[1 0 0],[0 1 0]).surf(hca);

hca = h(isub); isub = isub + 1;
[h_surf,h_ax] = ePDist1(490).reduce('2D',[1 0 0],[0 1 0]).surf(hca);

hca = h(isub); isub = isub + 1;
[h_surf] = ePDist1(490).reduce('2D',[1 0 0],[0 1 0]).surf(hca);

hca = h(isub); isub = isub + 1;
[h_surf,h_ax,h_all] = ePDist1.reduce('2D',[1 0 0],[0 1 0],'t',ePDist1.time([1 end])).surf(hca,'printinfo');

%%

times = ePDist1([1 end]).time; % nargout = 2 in subsref, working
ePDist1([1 end]).time % nargout = 2 in subsref, not working
gseB1([1 end]).time
