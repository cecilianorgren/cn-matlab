function varargout = ExB(lr,lz,phi0,B0,x0,y0,vtxy,vtz) 
...
y0 = y0*1e3; % km -> m
B0 = B0*1e-9; % nT -> T
vtz = vtz*1e3;     % km/s -> m/s
vtxy = vtxy*1e3;   % km/s -> m/s
...
y(1) = y0;   % m 
z(1) = -L/2; % m
vtangle = rand*360; % degrees
vx(1) = cosd(vtangle)*vtxy;     % m/s
vy(1) = sind(vtangle)*vtxy;     % m/s
vz(1) = vtz;                    % m/s

% Model electric field. !!! This is too slow, wrote expression direct
% instead in loop
if 0
...
Ez0 = @(x,y,z,lr,lz,phi0)(z/lz^2).*phi0.*exp(-0.5*(x/lr).^2-0.5*(y/lr).^2-0.5*(z/lz).^2);
