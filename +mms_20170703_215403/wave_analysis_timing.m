%
% NOTE: We dont have MMS4 data for this day!!!
%
...
c_eval('totE? = gseE?.tlim(tint_vicinity).resample(timeline);',ic)
c_eval('totE? = totE?.resample(timeline);',ic)
c_eval('E? = gseE?par.tlim(tint_vicinity).resample(timeline);',ic)
c_eval('E? = E?.resample(timeline);',ic)
c_eval('R? = gseR?.resample(timeline).tlim(tint);',ic)

dt_sampling = E1.time(2)-E1.time(1);
dt = nan(4,1);
C = nan(4,1);
for ic_ = 1:3
  c_eval('[tmpC,lags] = xcorr(E1.data,E?.data,''coeff'');',ic_)  
...
avBnorm = gseAvB.resample(gseVe1).resample(t_center).norm.data;
c_eval('dR? = gseR?.resample(t_center)-gseR1.resample(t_center); dRpar(?) = dR?.dot(gseAvB.resample(t_center).norm).data;',ic)
v_xcorr_all = dRpar./dt;
v_xcorr = mean(v_xcorr_all(2:3))*avBnorm;
v_direction = irf_norm(v_xcorr);
v_amplitude = sqrt(sum(v_xcorr.^2));
%c_eval('tsV?_timing = irf.ts_vec_xyz(totE?.time,repmat(v_xcorr,totE?.length,1));',ic)

% figure(33)
% h = irf_plot(2);
% isub = 1;
% hca = h(isub); isub = isub + 1;
% irf_plot(hca,{E1,E2,E3},'comp');
% hca = h(isub); isub = isub + 1;
% irf_plot(hca,{E1,E2,E3},'comp','dt',dt(1:3));


if 0 % integrate total E and dit with v
  c_eval('gseEdt? = irf_integrate(totE?,tint_esw(1));',ic);
  c_eval('gseEdt? = irf.ts_vec_xyz(gseEdt?.time,gseEdt?.data);',ic)
  phi_filt = 3;
  c_eval('gsePhi? = gseEdt?.dot(tsV?_timing); gsePhi?_filt = gsePhi?.filt(phi_filt,0,[],10);',ic)
  c_eval('gsePhi?_detrend = gsePhi?; gsePhi?_detrend.data = detrend(gsePhi?_detrend.data,''linear'');',ic)
else % integrate only Epar and multiply with v_amplitude
  c_eval('gseEdt? = irf_integrate(E?,tint_esw(1));',ic);  
  c_eval('gsePhi? = gseEdt?*v_amplitude*double(sign(v_direction(1)));',ic)
  c_eval('gsePhi?_detrend = gsePhi?; gsePhi?_detrend.data = detrend(gsePhi?_detrend.data,''linear'');',ic)
end
  
c_eval('v_trapping? = sqrt(2*units.e*gsePhi?.abs/units.me)*1e-3;',ic)
...
esw.phi_max = [max(findpeaks(gsePhi1.tlim(tint_esw).data)) max(findpeaks(gsePhi2.tlim(tint_esw).data)) max(findpeaks(gsePhi3.tlim(tint_esw).data))];
...
set(hca,'ColorOrder',mms_colors('1234'))
irf_legend(hca,{'mms1','mms2','mms3'},[0.98 0.9],'fontsize',12);  

hca = irf_panel('E dt');
set(hca,'ColorOrder',mms_colors('1234'))
irf_plot(hca,{E1,E2,E3},'comp','dt',dt)
hca.YLabel.String = {'E','(mV/m)'};
...
irf_legend(hca,{sprintf('v_{||}xB/|B| = %.1f x [%.2f %.2f %.2f] km/s',v_amplitude,avB)},[0.95 0.05],'fontsize',12);
set(hca,'ColorOrder',mms_colors('1'))
...
if 1 % Ex
  hca = irf_panel('Ex');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{gseE1.tlim(tint_vicinity).x,gseE2.tlim(tint_vicinity).x,gseE3.tlim(tint_vicinity).x},'comp')
...
if 1 % Ey
  hca = irf_panel('Ey');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{gseE1.tlim(tint_vicinity).y,gseE2.tlim(tint_vicinity).y,gseE3.tlim(tint_vicinity).y},'comp')
...
if 1 % Ez
  hca = irf_panel('Ez');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{gseE1.tlim(tint_vicinity).z,gseE2.tlim(tint_vicinity).z,gseE3.tlim(tint_vicinity).z},'comp')
...
if 1
  ...
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{gsePhi1,gsePhi2,gsePhi3},'comp','dt',dt)
  hca.YLabel.String = {'\Phi','(V)'};
end
if 0
  hca = irf_panel('phi dt detrend');
  set(hca,'ColorOrder',mms_colors('1234'))
  irf_plot(hca,{gsePhi1_detrend,gsePhi2_detrend,gsePhi3_detrend},'comp','dt',dt)
  hca.YLabel.String = {'\Phi','(V)'};
end
...
set(hca,'ColorOrder',mms_colors('1234'))
irf_plot(hca,{v_trapping1*1e-3,v_trapping2*1e-3,v_trapping3*1e-3},'comp','dt',dt)
...
data_format = '%s %s %s %s %s %.1f %.1f %.1f %.1f %4.0f %4.0f %4.0f %4.0f %2.1f %.3f %.3f %.3f %.3f %.1f %.1f'; 
...
data_format_read = '%s %s %s %s %s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f'; 
