...
if 1 % initialize figure
    ...
    tah=0.10;
    tah4=0.10;
    h(1)=axes('position',[0.15 tah4+4*(tah+0.005) 0.7 tah]); % [x y dx dy]
    h(2)=axes('position',[0.15 tah4+3*(tah+0.005) 0.7 tah]); % [x y dx dy]
    h(3)=axes('position',[0.15 tah4+2*(tah+0.005) 0.7 tah]); % [x y dx dy]
    h(4)=axes('position',[0.15 tah4+1*(tah+0.005) 0.7 tah]); % [x y dx dy]
    h(5)=axes('position',[0.15 tah4 0.7 tah]); % [x y dx dy]
    
    p(1)=axes('position',[0.15 tah4+5*(tah+0.005)+0.03 0.3*1.1 0.3*1.1]); % [x y dx dy]
    %axes(p(1),square)
...
if 1 % add sc configuration
    ...
    c_eval('arVi?=cn_toepoch(t1,t2,codVi?hr);',3:4);
    ...
    % Add B_0
    bcol=[0.6 0 0];
    bdot  = plot(p(1),6,-3,'color',bcol,'linestyle','o','markersize',2,'linewidth',1);
    bring = plot(p(1),6,-3,'color',bcol,'linestyle','o','markersize',20,'linewidth',1.1);
    b_txt = text(3.5,-3,'B','color',bcol);
    % Boundary normal
    ncol=[1 0 0.7];
    q1=quiver(6,-1.5,0,1,3,'color',ncol); hold on;  
    nstr(1)={'boundary'};
    nstr(2)={'normal'};
    bn_txt=text(1.6,0.5,nstr,'color',ncol);
    % Vi_av
    mvi=(arM*vi_av')';
    mvi_hat=(arM*vi_hat')';
    V_par=mvi(3);
    V_per=sqrt(mvi(1)^2+mvi(2)^2);
    q2=quiver(0,0,mvi_hat(1),mvi_hat(2),4*V_per/Vith,'color',[0.5 0.8 0.1]); hold on; 
    ...
    mExB_par=mExB_av(3);
    q3=quiver(0,0,mExB_hat(1),mExB_hat(2),4*mExB_per/Vith,'color',[1 0.6 0],'linewidth',1.1); hold on; 
    exbstr(1)={'ExB- '};
    exbstr(2)={'velocity'};
    ...
    bn_hat=arM*n_hat';
    kcol=[0.5 0 1];
    q4=quiver(0,0,-vchoice,0,4/Vith,'color',[0.5 0 1]); hold on;
    kstr(1)={'wave'};
    kstr(2)={'velocity'};
...
if 1 % add info to figure
    ...
    nf=nf+1;infostr(nf)={['n_{e} = ',num2str(Neav,'%.2f'),' cm^{-3}     \beta=',num2str(beta,'%0.1f')]};
    ...
    ann=annotation('textbox',[0.55 tah4+6*(tah+0.005)+0.20 0.5 0.2],...
...
if 1 % Potential and dB C3
    ...
    philab=ylabel(hca,['\phi_{\delta E}/T_e']);
    set(hca,'ColorOrder',[0 0 0]);
    irf_legend(hca,{['C','3']},[0.92 0.9])
...
if 1 % Potential and dB C4
    ...
    set(hca,'ColorOrder',[[0.8 0.5 0.2];[0.4 0.2 0.8];[0 0 0]]);
    irf_legend(hca,{'\phi_{\delta E}','\phi_{\delta B} and \delta B'},[0.02 0.1])
    philab=ylabel(hca,['      \phi_{\delta B}/T_e   and']);
    set(hca,'ColorOrder',[0 0 0]);
    irf_legend(hca,{['C','4']},[0.92 0.9])
    if 1 % Ytick and Yticklabel B nT
        %set(gca,'Box','off');
        axesPosition=get(hcp,'Position');
        yticks0=get(hcp,'ytick');
...
if 1 % Gradient length scale
    ...
    irf_legend(hca,{ymstr},[0.17 0.9],'FontSize',10,'color','black')
end
%
%irf_zoom('x',[cn_toepoch(t1) cn_toepoch(t2)]);
irf_zoom(h(1:np),'x',[phi3(1,1) phi3(end,1)])
