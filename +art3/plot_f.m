function [ax,varargout]=plot_f(varargin)
%art3.plot_f
%plot_f(ax,n,m,t,vd,d,a1,a2,pitchangles,plotoption,title_option)
% Usage:
...
if nargin<8
	contour3(ax,vp,vz,log10(f),30);view(0,90);
	axis equal;
	grid on;
...
%	Etot=1e3*1/3.517388e14*1*vtot.^2;	%normalized to first species, Etot[eV]
    ...
    if plotoption==1
        ...
        grid(ax,'on')
        xlabel(ax,'Etot [eV]')
        if QJAS_UNITS, ylabel(ax,'PSD [s^3/km^6]')
        else ylabel(ax,'PSD [s^3/m^6]')
        end
        legend(ax,M)
    elseif plotoption==2
        % E_reduced=(1/e)*mm(1)/2*vz_reduced.^2;	%normalized to first species, Etot[eV]
        semilogy(ax,vz_reduced,F_reduced);
        grid on
        xlabel(ax,'vz_{reduced} [m/s]')
        ylabel(ax,'F_{reduced} [[m/s]^{-1} m^{-3}]')
    else % default f(v)
        ...
        if QJAS_UNITS
            xlabel(ax,'Vtot [km/s]')
            ylabel(ax,'PSD [s^3/km^6]')
        else
            xlabel(ax,'Vtot [m/s]')
            ylabel(ax,'PSD [s^3/m^6]')
        end
        legend(M)
    end
end
if exist('title_option','var'),
  if ischar(title_option),
    title(ax,title_option);
  elseif title_option==1,
    title(ax,['vt(1)=' num2str(vt(1)) ', vd(1)=' num2str(vd(1)) ', d(1)=' num2str(d(1)) ', a1(1)=' num2str(a1(1)) ', a2(1)=' num2str(a2(1)) ]);
  end
else
  title(ax,['vt(1)=' num2str(vt(1)) ', vd(1)=' num2str(vd(1)) ', d(1)=' num2str(d(1)) ', a1(1)=' num2str(a1(1)) ', a2(1)=' num2str(a2(1)) ]);
end
