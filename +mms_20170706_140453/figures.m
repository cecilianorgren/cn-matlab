...
tintZoom = irf.tint('2017-07-06T14:05:43.00Z',15);
eint = [000 40000];
vint = [-Inf Inf];
...
eLine = gseB1.resample(eDist).norm;
iLine = gseB1.resample(iDist).norm;

ef1D = eDist.reduce('1D',eLine,'vint',vint,'scpot',scpot_lim); % reduced distribution along B
if1D = iDist.reduce('1D',iLine,'vint',vint); % reduced distribution along B
...
if 1 % iDEF omni
  isub = isub + 1;
  hca = irf_panel('iDEF');  
  [hout,hcb] = irf_spectrogram(hca,iDist.omni.specrec,'log');
%   hold(hca,'on')
...
if 1 % i psd vpar
  isub = isub + 1;
  hca = irf_panel('iLine');
  irf_plot(hca,if1D.specrec('velocity_1D'));
  hold(hca,'on')
